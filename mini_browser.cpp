...
// ADDED: Google Login Bypass, Working Pin Button, Pure Popup Mode, AI Filter Integration.
...
std::wstring GetBlocked_HTML(bool isDark) {
    ...
    L"  '\"ক্ষণিকের আনন্দের জন্য নিজের ভবিষ্যৎ নষ্ট করো না।\" - সংগৃহীত',"
...
std::wstring GetAiInjectScript(const std::wstring& currentUrl) {
    ...
    // Read the settings from file generated by tab_ai.cpp
...
struct BrowserWindowData {
    ...
    bool isPinned = false; // 🟢 Working Pin logic Variable
...
static int NavTotalH(HWND hWnd) {
    ...
    // 🟢 Popup Mode Frame Logic: Hide Addressbar, Toolbar, Tabs
...
static void DrawBrowserContent(HWND hWnd, HDC hdc) {
    ...
    // Window controls & Titlebar Icons
    {
        ...
        // 🟢 Pin icon dynamically updates based on 'isPinned'
    ...
    // 🟢 HIDE Toolbar, Tabs, Bookmarks in Popup Mode
    if (!g_isPureViewerMode) {
        // Tab strip
        {
...
public:
    ...
    HRESULT STDMETHODCALLTYPE Invoke(HRESULT hr, ICoreWebView2Controller* ctl) override {
        ...
        // 🟢 DIRECT LOCAL NTP LOAD ON STARTUP (Unless passing pure viewer URL)
...
LRESULT CALLBACK ViewerWndProc(HWND hWnd, UINT msg, WPARAM wParam, LPARAM lParam) {
    ...
    case WM_NCHITTEST: {
        ...
        if (def == HTNOWHERE || def == HTCLIENT) {
            ...
            if (!g_windows.count(hWnd) || !g_windows[hWnd].isFullScreen) {
                ...
                // 🟢 Support dragging titlebar in pure popup mode
                if (g_isPureViewerMode) {
    ...
    case WM_LBUTTONDBLCLK: {
        if (!g_windows.count(hWnd)) break;
        if (g_isPureViewerMode) break; // Disable double-click tab creation in popup
        UINT dpi = GetWndDpi(hWnd);
...
void LaunchMiniBrowser(std::wstring url, std::wstring /*title*/) {
    ...
    // 🟢 LOCAL NTP GUARANTEE FOR NORMAL MODE
    TabData firstTab;
