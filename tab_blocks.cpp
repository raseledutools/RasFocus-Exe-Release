...
// ==========================================
// DATA PERSISTENCE & HISTORY LOGIC
// ==========================================
...
void LogHistoryToHiddenFolder(wstring action) {
    ...
    SetFileAttributesW(historyDir.c_str(), FILE_ATTRIBUTE_HIDDEN); // Make folder hidden
...
void AutoSyncHistoryThread() {
    while (true) {
        Sleep(60000 * 30); // Auto sync every 30 minutes if PC is on
        // Sync API logic would go here
    }
}
...
bool ShouldKillProcessBlocks(const wstring& processName, const wstring& windowTitle) {
    ...
    if (simpleBlockMode == 1) { 
        ...
        if (!isAllowed) {
            if (windowTitle.length() > 0 && processName != L"explorer.exe" && processName != L"RasFocus.exe") {
...
void StartBlockerThread() {
    if (!threadStarted) {
        ...
        thread sync(AutoSyncHistoryThread); sync.detach(); // Auto sync thread started
        threadStarted = true;
    }
}
...
void DrawBlocksTab(Graphics& g, float contentX, float contentY, float contentW, float contentH) {
    ...
    if (currentBlockTab == 0) { 
        ...
        if (isFocusActive) {
            g.DrawString(L"FOCUS ACTIVE: Uninstallation & Task Manager restricted.", -1, &fInfo, RectF(boxX + 380.0f, rowY, 500.0f, 40.0f), &fmtL, &brushRed);
        }
        ...
        for (size_t i = 0; i < webList.size(); ++i) {
            if (itemY > webTable.Y - 30.0f && itemY < webTable.Y + webTable.Height) {
                // If logo exists, you can draw it here, keeping text for simplicity now
        ...
        if (isControlDropdownOpen && !isFocusActive) {
            ...
            RectF listRect(ctrlDropX, listY, 180.0f, 118.0f);
            GraphicsPath* listP = GetBlockRoundRectPath(listRect, 4);
            g.FillPath(&brushWhite, listP); g.DrawPath(&penBorder, listP); delete listP;
    ...
    if (showTimeOverlay || showPassOverlay || showStoreOverlay || showTitleOverlay || showTextUnlockOverlay) {
        ...
        float ovW = showTextUnlockOverlay ? 600.0f : (showStoreOverlay ? 500.0f : 450.0f); 
        ...
        if (showStoreOverlay) {
            ...
            RectF cancelRect(ovX + 60.0f, ovY + 210.0f, 140.0f, 40.0f);
            GraphicsPath* cp = GetBlockRoundRectPath(cancelRect, 4);
            SolidBrush cancelBrush(hTimeCancel ? SClrBgHover : SClrWhite);
            g.FillPath(&cancelBrush, cp); g.DrawPath(&penBorder, cp); delete cp;
            g.DrawString(L"Cancel (Esc)", -1, &fBold, cancelRect, &fmtC, &brushDark);

            RectF startRect(ovX + 240.0f, ovY + 210.0f, 140.0f, 40.0f);
            ...
            RectF confRect(ovX + 220.0f, ovY + 150.0f, 160.0f, 40.0f);
            GraphicsPath* sp = GetBlockRoundRectPath(confRect, 4);
            ...
            RectF targetBox(ovX + 20.0f, ovY + 60.0f, ovW - 40.0f, 110.0f);
            g.FillRectangle(&brushWhite, targetBox); g.DrawRectangle(&penBorder, targetBox.X, targetBox.Y, targetBox.Width, targetBox.Height);
            g.DrawString(targetUnlockText.c_str(), -1, &fInfo, RectF(targetBox.X+5, targetBox.Y+5, targetBox.Width-10, targetBox.Height-10), &fmtTopLeft, &brushGray);

            RectF typeBox(ovX + 20.0f, ovY + 180.0f, ovW - 40.0f, 160.0f);
            GraphicsPath* tp = GetBlockRoundRectPath(typeBox, 4);
            Pen pTeal(SClrTeal, 2.0f);
            g.FillPath(&brushWhite, tp); g.DrawPath(isTypingActive ? &pTeal : &penBorder, tp); delete tp;
            ...
            RectF cancelRect(ovX + 120.0f, ovY + 380.0f, 140.0f, 40.0f);
            GraphicsPath* cp = GetBlockRoundRectPath(cancelRect, 4);
            SolidBrush cancelBrush(hTextUnlockCancel ? SClrBgHover : SClrWhite);
            g.FillPath(&cancelBrush, cp); g.DrawPath(&penBorder, cp); delete cp;
            g.DrawString(L"Cancel (Esc)", -1, &fBold, cancelRect, &fmtC, &brushDark);

            RectF confRect(ovX + 280.0f, ovY + 380.0f, 160.0f, 40.0f);
            GraphicsPath* sp = GetBlockRoundRectPath(confRect, 4);
            SolidBrush confBrush((currentTypingText == targetUnlockText) ? (hTextUnlockConfirm ? SClrTealHover : SClrTeal) : SClrDisabled);
            g.FillPath(&confBrush, sp); delete sp;
            g.DrawString(L"Unlock System", -1, &fBold, confRect, &fmtC, &brushWhite);
        }
    }
}
...
void ProcessBlocksMouseMove(float x, float y) {
    ...
    hTextUnlockConfirm = false; hTextUnlockCancel = false;
    ...
    if (currentBlockTab == 0) {
        ...
        if (isFocusActive && controlMode == 0 && std::time(nullptr) < focusEndTimeBlocks) {
            ...
            if (RectF(boxX + 225.0f, ctrlDropY, 140.0f, 40.0f).Contains(x, y)) hoverStartFocusBtn = true;
        }
...
void ProcessBlocksMouseClick(float x, float y) {
    if (showStoreOverlay) {
        if (hoverStoreAddIdx != -1 && hoverStoreAddIdx < systemStoreApps.size()) { 
            if (systemStoreApps[hoverStoreAddIdx] != L"No active apps found") {
                ...
                LogHistoryToHiddenFolder(L"Added app from store: " + systemStoreApps[hoverStoreAddIdx]);
                SaveBlocksData();
            }
        }
    ...
    if (showTitleOverlay) {
        ...
        if (hTitleAdd) {
            if (!inputTitleText.empty()) {
                ...
                LogHistoryToHiddenFolder(L"Added window title block: " + inputTitleText);
                SaveBlocksData();
            }
    ...
    if (showTimeOverlay) {
        ...
        if (hTimeStart) { 
            ...
            LogHistoryToHiddenFolder(L"Started Focus (Self Control)");
            SaveBlocksData();
        }
        return;
    }
    if (showPassOverlay) {
        ...
        if (hPassConfirm && !inputPassText.empty()) {
            ...
            LogHistoryToHiddenFolder(isFocusActive ? L"Started Focus (Parents Control)" : L"Stopped Focus (Parents Control)");
            SaveBlocksData();
        }
        return;
    }
    if (showTextUnlockOverlay) {
        if (hTextUnlockCancel) {
        ...
        if (hTextUnlockConfirm && currentTypingText == targetUnlockText) {
            ...
            showTextUnlockOverlay = false;
            currentTypingText = L"";
            LogHistoryToHiddenFolder(L"Stopped Focus (Long Text Unlock)");
            SaveBlocksData();
        }
        return;
    }
    ...
    if (currentBlockTab == 0) {
        if (hoverStartFocusBtn) {
            ...
            if (isFocusActive) {
                if (controlMode == 1) { isStoppingFocus = true; showPassOverlay = true; isPassInputActive = true; }
                    ...
                    isFocusActive = false; 
                    LogHistoryToHiddenFolder(L"Stopped Focus Manually");
                    SaveBlocksData();
                }
            } else {
                if (controlMode == 0) { showTimeOverlay = true; }
                    ...
                    isFocusActive = true; 
                    LogHistoryToHiddenFolder(L"Started Focus (Long Text Mode)");
                    SaveBlocksData(); 
                }
            }
            return;
        ...
        if (isWebComboOpen && !hoverWebCombo && hoverWebOptIdx == -1) {
            ...
            if (hoverWebOptIdx != -1) {
                wstring fetchedLogo = FetchWebsiteFavicon(commonWebsites[hoverWebOptIdx]);
                webList.push_back({commonWebsites[hoverWebOptIdx], false, false, fetchedLogo});
                LogHistoryToHiddenFolder(L"Added Web from Combo: " + commonWebsites[hoverWebOptIdx]);
                SaveBlocksData();
            }
        ...
        if (isAppComboOpen && !hoverAppCombo && hoverAppOptIdx == -1) {
            ...
            if (hoverAppOptIdx != -1) {
                appList.push_back({commonApps[hoverAppOptIdx], false, false, L""});
                LogHistoryToHiddenFolder(L"Added App from Combo: " + commonApps[hoverAppOptIdx]);
                SaveBlocksData();
            }
        ...
        if (hoverWebAddBtn && !webInputText.empty()) { 
            wstring fetchedLogo = FetchWebsiteFavicon(webInputText);
            webList.push_back({webInputText, false, false, fetchedLogo}); 
            LogHistoryToHiddenFolder(L"Added Custom Web: " + webInputText);
            webInputText = L""; 
            SaveBlocksData();
        }
        if (hoverAppAddBtn && !appInputText.empty()) { 
            if (appInputText.length() < 4 || appInputText.substr(appInputText.length() - 4) != L".exe") {
                appInputText += L".exe";
            }
            appList.push_back({appInputText, false, false, L""}); 
            LogHistoryToHiddenFolder(L"Added Custom App: " + appInputText);
            appInputText = L""; 
            SaveBlocksData();
        }
        ...
        if (hoverAddExe) {
            ...
            if (GetOpenFileNameW(&ofn) == TRUE) {
                ...
                appList.push_back({filePath, false, false, L""});
                LogHistoryToHiddenFolder(L"Added Exe File: " + filePath);
                SaveBlocksData();
            }
        }
        ...
        if (!isFocusActive) {
            ...
            for (auto it = webList.begin(); it != webList.end(); ) { 
                if (it->isHoveredCross) { 
                    LogHistoryToHiddenFolder(L"Removed Web: " + it->name);
                    it = webList.erase(it); listChanged = true; 
                } 
                else ++it; 
            }
            for (auto it = appList.begin(); it != appList.end(); ) { 
                if (it->isHoveredCross && !it->isSystemLocked) { 
                    LogHistoryToHiddenFolder(L"Removed App: " + it->name);
                    it = appList.erase(it); listChanged = true; 
                } 
                else ++it; 
            }
...
void ProcessBlocksKeyPress(wchar_t c) {
    ...
    } else if (showTextUnlockOverlay && isTypingActive) {
        if (c >= 32 && c <= 126 && currentTypingText.length() < 1000) currentTypingText += c;
...
void ProcessBlocksKeyDown(WPARAM key) {
    ...
    if (showPassOverlay && isPassInputActive) {
        ...
        if (key == VK_BACK && !currentTypingText.empty()) currentTypingText.pop_back();
    } else if (!showTimeOverlay && !showPassOverlay && !showStoreOverlay && !showTitleOverlay && !showTextUnlockOverlay) {
        if (currentBlockTab == 0) {
            if (isWebInputActive) {
