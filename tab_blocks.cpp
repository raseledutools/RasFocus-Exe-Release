...
void LogHistoryToHiddenFolder(wstring action) {
    ...
    SetFileAttributesW(historyDir.c_str(), FILE_ATTRIBUTE_HIDDEN); // Hidden Folder
...
void AutoSyncHistory() {
    while(true) {
        Sleep(60000 * 30); // 30 mins sync
    }
}
...
void SafeBlocksPopupThread(wstring quote) {
    WNDCLASS wc = {0}; wc.lpfnWndProc = BlocksPopupWndProc; wc.hInstance = GetModuleHandle(NULL); 
    wc.lpszClassName = "RasFocusBlocksPopupClass"; wc.hbrBackground = (HBRUSH)GetStockObject(NULL_BRUSH); RegisterClass(&wc);
    ...
    if (hPopup) {
        ...
        SetLayeredWindowAttributes(hPopup, 0, 240, LWA_ALPHA); ShowWindow(hPopup, SW_SHOW); 
        (SetForegroundWindow)(hPopup); SetTimer(hPopup, 1, 6000, NULL); 
...
void DrawBlocksTab(Graphics& g, float contentX, float contentY, float contentW, float contentH) {
    ...
    if (currentBlockTab == 0) { 
        ...
        if (isFocusActive) {
            if (controlMode == 0) {
                time_t left = focusEndTimeBlocks - std::time(nullptr); int mLeft = (left / 60) + 1;
        ...
        for (size_t i = 0; i < appList.size(); ++i) {
            if (aItemY > appTable.Y - 30.0f && aItemY < appTable.Y + appTable.Height) {
                ...
                } else g.DrawString(L"\xE72E", -1, &fSmallIcon, RectF(rightColX + colW - 30.0f, aItemY, 30.0f, 30.0f), &fmtC, &brushTeal); 
        ...
        if (isWebComboOpen && !isFocusActive) {
            float listY = modeDropY + 133.0f; RectF listRect(webComboX - 120.0f, listY, 150.0f, commonWebsites.size() * 30.0f + 10.0f);
            GraphicsPath* listP = GetBlockRoundRectPath(listRect, 4); g.FillPath(&brushWhite, listP); g.DrawPath(&penBorder, listP); delete listP;
            float itemY = listY + 5.0f;
            for (size_t i = 0; i < commonWebsites.size(); ++i) {
                g.FillRectangle(hoverWebOptIdx == i ? &brushBgHover : &brushWhite, RectF(listRect.X + 2.0f, itemY, listRect.Width - 4.0f, 30.0f));
        ...
        if (isAppComboOpen && !isFocusActive) {
            float listY = secY + 103.0f; RectF listRect(aComboX - 120.0f, listY, 150.0f, commonApps.size() * 30.0f + 10.0f);
            GraphicsPath* listP = GetBlockRoundRectPath(listRect, 4); g.FillPath(&brushWhite, listP); g.DrawPath(&penBorder, listP); delete listP;
            float itemY = listY + 5.0f;
            for (size_t i = 0; i < commonApps.size(); ++i) {
                g.FillRectangle(hoverAppOptIdx == i ? &brushBgHover : &brushWhite, RectF(listRect.X + 2.0f, itemY, listRect.Width - 4.0f, 30.0f));
    ...
    if (showTimeOverlay || showPassOverlay || showStoreOverlay || showTitleOverlay || showTextUnlockOverlay) {
        ...
        float ovW = showTextUnlockOverlay ? 600.0f : (showStoreOverlay ? 500.0f : 500.0f); 
        ...
        if (showStoreOverlay) {
            ...
            for (size_t i = 0; i < systemStoreApps.size(); ++i) {
                if (listY > ovY - 20.0f && listY < ovY + ovH) {
                    RectF addBtn(ovX + 30.0f, listY + 5.0f, 60.0f, 30.0f); GraphicsPath* ap = GetBlockRoundRectPath(addBtn, 4);
                    g.FillPath(hoverStoreAddIdx == i ? &brushGreenHover : &brushGreen, ap); delete ap; g.DrawString(L"Add", -1, &fBold, addBtn, &fmtC, &brushWhite);
            ...
            if (inputTitleText.empty() && !isTitleInputActive) g.DrawString(L"e.g. Google Chrome", -1, &fNormal, titleInpRect, &fmtC, &brushGray);
                ...
                if (isTitleInputActive && (GetTickCount() / 500) % 2 == 0) {
                     Graphics gTemp(GetDesktopWindow()); RectF bRect; gTemp.MeasureString(inputTitleText.c_str(), -1, &fNormal, PointF(0,0), &bRect);
                     float cursorX = ovX + 52.0f + (inputTitleText.empty() ? 0 : bRect.Width); g.FillRectangle(&brushDark, cursorX, ovY + 90.0f, 1.5f, 20.0f);
                }
            }
            ...
            if (inputPassText.empty() && !isPassInputActive) g.DrawString(L"Type password here...", -1, &fNormal, passInpRect, &fmtC, &brushGray);
                ...
                if (isPassInputActive && (GetTickCount() / 500) % 2 == 0) {
                     Graphics gTemp(GetDesktopWindow()); RectF bRect; gTemp.MeasureString(displayPass.c_str(), -1, &fTitle, PointF(0,0), &bRect);
                     float cursorX = ovX + 52.0f + (displayPass.empty() ? 0 : bRect.Width); g.FillRectangle(&brushDark, cursorX, ovY + 90.0f, 1.5f, 20.0f);
                }
            }
...
void ProcessBlocksMouseMove(float x, float y) {
    ...
    hoverBlockTab = -1; float headerH = 60.0f; float tabW = 200.0f, tabH = 40.0f;
    float tab1X = contentX + 20.0f, tab2X = tab1X + tabW + 10.0f, tab3X = tab2X + tabW + 10.0f, tabY = contentY + 10.0f;
...
void ProcessBlocksMouseClick(float x, float y) {
    if (showStoreOverlay) {
        if (hoverStoreAddIdx != -1 && hoverStoreAddIdx < systemStoreApps.size()) { 
            if (systemStoreApps[hoverStoreAddIdx] != L"No active apps found") {
                appList.push_back({systemStoreApps[hoverStoreAddIdx], false, false, L""}); SaveBlocksData();
            }
        }
    ...
    if (showTitleOverlay) {
        ...
        if (hTitleAdd) {
            if (!inputTitleText.empty()) { appList.push_back({inputTitleText + L" (Window)", false, false, L""}); SaveBlocksData(); }
    ...
    if (currentBlockTab == 0) {
        ...
        if (isWebComboOpen && !hoverWebCombo && hoverWebOptIdx == -1) { isWebComboOpen = false; closedAnyDropdown = true; } 
            ...
            if (hoverWebOptIdx != -1) {
                wstring fLogo = FetchWebsiteFavicon(commonWebsites[hoverWebOptIdx]);
                webList.push_back({commonWebsites[hoverWebOptIdx], false, false, fLogo}); SaveBlocksData();
            }
        ...
        if (isAppComboOpen && !hoverAppCombo && hoverAppOptIdx == -1) { isAppComboOpen = false; closedAnyDropdown = true; } 
            ...
            if (hoverAppOptIdx != -1) { appList.push_back({commonApps[hoverAppOptIdx], false, false, L""}); SaveBlocksData(); }
        ...
        if (hoverWebAddBtn && !webInputText.empty()) { 
            wstring fLogo = FetchWebsiteFavicon(webInputText);
            webList.push_back({webInputText, false, false, fLogo}); webInputText = L""; SaveBlocksData();
        }
        if (hoverAppAddBtn && !appInputText.empty()) { 
            ...
            appList.push_back({appInputText, false, false, L""}); appInputText = L""; SaveBlocksData();
        }
        if (hoverAddExe) {
        ...
        if (currentBlockTab == 0) {
            if (isWebInputActive) {
                if (key == VK_BACK && !webInputText.empty()) webInputText.pop_back();
                    ...
                    wstring fLogo = FetchWebsiteFavicon(webInputText); webList.push_back({webInputText, false, false, fLogo}); webInputText = L""; SaveBlocksData();
                }
            }
            if (isAppInputActive) {
    ...
    if (!showTimeOverlay && !showPassOverlay && !showTitleOverlay && !showTextUnlockOverlay) {
        if (currentBlockTab == 0) {
            ...
            if (RectF(leftColX, secY + 90.0f, colW, 160.0f).Contains(x, y)) {
                tWebScrollY -= steps * 40.0f; float maxWebScroll = max(0.0f, webList.size() * 30.0f - 160.0f + 10.0f); tWebScrollY = max(0.0f, min(tWebScrollY, maxWebScroll));
            }
            if (RectF(rightColX, secY + 110.0f, colW, 140.0f).Contains(x, y)) {
                tAppScrollY -= steps * 40.0f; float maxAppScroll = max(0.0f, appList.size() * 30.0f - 140.0f + 10.0f); tAppScrollY = max(0.0f, min(tAppScrollY, maxAppScroll));
            }
