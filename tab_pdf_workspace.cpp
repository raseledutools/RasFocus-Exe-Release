// ============================================================
//  AcrobatHTML_Full.cpp  (MSVC C2026-safe edition)
//  Every raw-string literal is kept well under 16380 chars.
//  Consecutive ss << calls concatenate automatically.
// ============================================================
...
ss << LR"CSS(<style>
:root{
  --c-topbar:#1c1c1c;--c-tabbar:#2d2d2d;--c-toolbar:#f0f0f0;
  --c-toolbar-border:#d0d0d0;--c-viewer:#808080;--c-sidebar:#f5f5f5;
  --c-sidebar-border:#ddd;--c-panel:#fafafa;--c-accent:#c0392b;
  --c-accent-h:#a93226;--c-accent2:#2980b9;--c-text:#1a1a1a;
  --c-muted:#666;--c-light:#ccc;--c-border:#e0e0e0;
  --c-highlight:rgba(255,220,0,0.45);--c-pen:#e53935;--c-page:#fff;
  --topbar-h:30px;--tabbar-h:28px;--ribbon-h:88px;--statusbar-h:22px;
  --left-panel-w:170px;--right-panel-w:220px;
  --shadow:0 2px 12px rgba(0,0,0,.25);--radius:3px;
}
*{margin:0;padding:0;box-sizing:border-box;}
html,body{height:100vh;overflow:hidden;font-family:'Segoe UI',Arial,sans-serif;
  font-size:12px;background:var(--c-viewer);color:var(--c-text);user-select:none;}
...
// ─────────────────────────────────────────────────────────────
ss << LR"CSS(<style>
.topbar{height:var(--topbar-h);background:var(--c-topbar);display:flex;
  align-items:center;padding:0 6px;gap:1px;flex-shrink:0;
  border-bottom:1px solid #000;z-index:200;}
.top-menu{color:var(--c-light);font-size:11.5px;padding:2px 9px;border-radius:2px;
  cursor:pointer;line-height:var(--topbar-h);white-space:nowrap;position:relative;
...
.top-icon{color:var(--c-light);cursor:pointer;padding:2px 6px;border-radius:2px;
  font-size:14px;line-height:1;transition:background .1s;}
.top-icon:hover{background:rgba(255,255,255,.14);color:#fff;}
.dropdown{display:none;position:fixed;background:#2b2b2b;border:1px solid #444;
  border-radius:3px;box-shadow:0 4px 16px rgba(0,0,0,.4);z-index:9000;
  min-width:180px;padding:3px 0;}
.dropdown.show{display:block;}
.dd-item{color:#ccc;font-size:11.5px;padding:5px 14px;cursor:pointer;
...
// ─────────────────────────────────────────────────────────────
ss << LR"CSS(<style>
.tabbar{height:var(--tabbar-h);background:var(--c-tabbar);display:flex;
  align-items:flex-end;padding-left:6px;overflow-x:auto;flex-shrink:0;
  border-bottom:1px solid #111;}
.tabbar::-webkit-scrollbar{height:0;}
.pdf-tab{height:24px;background:#3c3c3c;color:#aaa;padding:0 8px;
  ...
  border-bottom:none;transition:background .1s;position:relative;top:1px;flex-shrink:0;}
...
.tab-close{font-size:13px;cursor:pointer;opacity:.5;border-radius:2px;
...
.tab-add{color:#999;cursor:pointer;padding:0 9px;font-size:16px;
...
// ─────────────────────────────────────────────────────────────
// PART 05a · Ribbon CSS (buttons + groups)
// ─────────────────────────────────────────────────────────────
ss << LR"CSS(<style>
...
.rtab{padding:4px 14px;font-size:11px;cursor:pointer;border-radius:2px 2px 0 0;
...
.rtab.active{background:var(--c-toolbar);border-color:var(--c-toolbar-border);
...
.rbtn{display:flex;flex-direction:column;align-items:center;justify-content:center;
...
// ─────────────────────────────────────────────────────────────
// PART 05b · Ribbon CSS (small buttons, inputs, swatches)
// ─────────────────────────────────────────────────────────────
ss << LR"CSS(<style>
.rbtn-sm{display:flex;align-items:center;gap:4px;cursor:pointer;border-radius:var(--radius);
...
.color-swatch{width:16px;height:16px;border-radius:2px;border:1px solid #bbb;
  cursor:pointer;display:inline-block;flex-shrink:0;}
.ribbon-num{width:42px;padding:2px 4px;border:1px solid var(--c-border);
  border-radius:var(--radius);font-size:11px;text-align:center;outline:none;background:#fff;}
.ribbon-num:focus{border-color:var(--c-accent2);}
select.ribbon-sel{padding:2px 4px;border:1px solid var(--c-border);border-radius:var(--radius);
...
.left-panel{width:var(--left-panel-w);background:#e8e8e8;border-right:1px solid var(--c-border);
...
.lp-tab{flex:1;text-align:center;padding:5px 0;font-size:9.5px;cursor:pointer;
  border-right:1px solid var(--c-border);color:var(--c-muted);transition:background .1s;
...
.thumb-item{background:#fff;border:2px solid transparent;border-radius:3px;
  cursor:pointer;position:relative;box-shadow:0 1px 4px rgba(0,0,0,.18);transition:border-color .12s;}
.thumb-item:hover{border-color:#aaa;}
.thumb-item.selected{border-color:var(--c-accent);}
.thumb-item canvas{width:100%;display:block;}
.thumb-pg-num{position:absolute;bottom:2px;left:0;right:0;text-align:center;
  font-size:8.5px;color:#555;background:rgba(255,255,255,.75);}
.thumb-del{position:absolute;top:2px;right:2px;width:14px;height:14px;
...
.bm-item{padding:5px 10px;font-size:11px;cursor:pointer;border-bottom:1px solid var(--c-border);
...
// ─────────────────────────────────────────────────────────────
ss << LR"CSS(<style>
.pdf-viewer-area{flex:1;overflow:auto;background:var(--c-viewer);
  display:flex;justify-content:center;position:relative;cursor:default;}
.pdf-container{display:flex;flex-direction:column;gap:14px;
  align-items:center;padding:20px;width:100%;}
.page-wrapper{position:relative;background:var(--c-page);box-shadow:var(--shadow);
  flex-shrink:0;display:block;}
.page-wrapper .pdf-canvas{display:block;position:relative;z-index:1;}
.page-wrapper .draw-canvas{position:absolute;top:0;left:0;z-index:2;pointer-events:none;}
.page-wrapper .draw-canvas.can-draw{pointer-events:auto;}
.page-wrapper .text-layer{position:absolute;top:0;left:0;z-index:3;pointer-events:none;
  overflow:hidden;opacity:.2;line-height:1;}
.ruler-h,.ruler-v{position:absolute;background:rgba(220,220,220,.9);pointer-events:none;
  z-index:20;font-size:8px;color:#888;overflow:hidden;}
.ruler-h{top:0;left:0;right:0;height:16px;}
.ruler-v{top:0;left:0;bottom:0;width:16px;}
.grid-overlay{position:absolute;inset:0;pointer-events:none;z-index:19;
...
.crop-overlay{position:absolute;inset:0;z-index:25;cursor:crosshair;
  background:rgba(0,0,0,.35);display:none;}
.crop-overlay.show{display:block;}
.crop-rect{position:absolute;border:2px solid var(--c-accent2);background:transparent;
  box-shadow:0 0 0 9999px rgba(0,0,0,.4);pointer-events:none;}
.sel-rect{position:absolute;border:1.5px dashed var(--c-accent2);
  background:rgba(41,128,185,.08);pointer-events:none;z-index:30;display:none;}
.redact-rect{position:absolute;background:#000;z-index:22;cursor:pointer;
  border:2px solid var(--c-accent);}
.stamp-el{position:absolute;z-index:18;pointer-events:auto;cursor:move;
  font-size:22px;font-weight:900;letter-spacing:2px;opacity:.55;
  border:3px solid;border-radius:4px;padding:2px 10px;text-transform:uppercase;}
.textbox-el{position:absolute;z-index:16;background:rgba(255,255,255,.85);
...
// ─────────────────────────────────────────────────────────────
ss << LR"CSS(<style>
.right-panel{width:var(--right-panel-w);background:var(--c-sidebar);
  border-left:1px solid var(--c-sidebar-border);
  display:flex;flex-direction:column;overflow-y:auto;flex-shrink:0;transition:width .18s;}
.right-panel.collapsed{width:0;border:none;overflow:hidden;}
.rp-section{border-bottom:1px solid var(--c-border);}
.rp-header{padding:6px 10px;font-size:10px;font-weight:700;color:var(--c-muted);
  text-transform:uppercase;letter-spacing:.5px;display:flex;align-items:center;
  gap:4px;cursor:pointer;background:#ececec;border-bottom:1px solid var(--c-border);}
...
.rp-input{flex:1;padding:3px 5px;border:1px solid var(--c-border);
  border-radius:var(--radius);font-size:11px;outline:none;min-width:50px;background:#fff;}
.rp-input:focus{border-color:var(--c-accent2);}
.rp-btn{width:100%;padding:5px 0;border:1px solid var(--c-border);border-radius:var(--radius);
  background:#fff;cursor:pointer;font-size:11px;font-weight:600;color:var(--c-text);
...
.badge{display:inline-block;background:var(--c-accent2);color:#fff;
...
// ─────────────────────────────────────────────────────────────
ss << LR"CSS(<style>
.statusbar{height:var(--statusbar-h);background:#e0e0e0;border-top:1px solid #ccc;
...
.sb-zoom-btn{width:16px;height:16px;background:#ccc;border:none;border-radius:2px;
  cursor:pointer;font-size:12px;display:flex;align-items:center;justify-content:center;line-height:1;}
...
.findbar{display:none;position:absolute;bottom:var(--statusbar-h);right:16px;
  background:#fff;border:1px solid var(--c-border);border-radius:4px;
  box-shadow:var(--shadow);padding:6px 8px;z-index:500;align-items:center;gap:6px;}
.findbar.show{display:flex;}
.findbar input{padding:3px 7px;border:1px solid var(--c-border);border-radius:var(--radius);
  font-size:11.5px;outline:none;width:180px;}
.findbar input:focus{border-color:var(--c-accent2);}
.find-btn{padding:3px 8px;border:1px solid var(--c-border);border-radius:var(--radius);
...
.toast-box{position:fixed;bottom:32px;left:50%;transform:translateX(-50%);
  z-index:9999;display:flex;flex-direction:column;gap:5px;pointer-events:none;}
.toast{padding:7px 16px;border-radius:4px;background:#2a2a2a;color:#fff;
...
.loading-overlay{display:none;position:fixed;inset:0;background:rgba(0,0,0,.55);
  z-index:9000;flex-direction:column;align-items:center;justify-content:center;color:#fff;}
.loading-overlay.show{display:flex;}
.spinner{width:30px;height:30px;border:3px solid rgba(255,255,255,.2);
...
.modal{background:#fff;border-radius:5px;padding:20px 22px;min-width:320px;max-width:520px;
...
.modal textarea,.modal select{width:100%;padding:6px 8px;border:1px solid var(--c-border);
  border-radius:var(--radius);font-size:12px;outline:none;margin-bottom:10px;
  font-family:inherit;background:#fff;}
.modal input:focus,.modal textarea:focus,.modal select:focus{border-color:var(--c-accent2);}
.modal textarea{resize:vertical;min-height:60px;}
.modal-actions{display:flex;gap:8px;justify-content:flex-end;margin-top:6px;}
.btn{padding:6px 16px;border:none;border-radius:var(--radius);cursor:pointer;
...
body.read .left-panel,body.read .right-panel,body.read .ribbon-wrap,
body.read .tabbar,body.read .topbar,body.read .statusbar{display:none!important;}
body.read .pdf-viewer-area{background:#1a1a1a!important;}
body.present *{cursor:none!important;}
body.present .left-panel,body.present .right-panel,body.present .ribbon-wrap,
...
// ─────────────────────────────────────────────────────────────
// PART 11 · Context menu + Colour picker + Sig modal CSS
// ─────────────────────────────────────────────────────────────
ss << LR"CSS(<style>
.ctx-menu{display:none;position:fixed;background:#fff;border:1px solid var(--c-border);
  border-radius:4px;box-shadow:0 4px 18px rgba(0,0,0,.2);z-index:7000;padding:3px 0;min-width:170px;}
.ctx-menu.show{display:block;}
.ctx-item{padding:5px 14px;font-size:11.5px;cursor:pointer;color:var(--c-text);
...
.color-picker-popup{display:none;position:fixed;background:#fff;border:1px solid var(--c-border);
  border-radius:4px;box-shadow:0 4px 16px rgba(0,0,0,.22);padding:10px;z-index:6000;}
.color-picker-popup.show{display:block;}
.color-grid{display:grid;grid-template-columns:repeat(8,18px);gap:3px;}
.color-cell{width:18px;height:18px;border-radius:2px;cursor:pointer;
  border:1px solid rgba(0,0,0,.12);transition:transform .1s;}
.color-cell:hover{transform:scale(1.25);z-index:1;position:relative;}
#sig-modal-canvas{border:1px solid var(--c-border);border-radius:3px;
...
// ─────────────────────────────────────────────────────────────
// PART 12b · File + Edit dropdown menus
// ─────────────────────────────────────────────────────────────
ss << LR"HTML(
...
// ─────────────────────────────────────────────────────────────
// PART 12c · View + Tools dropdown menus
// ─────────────────────────────────────────────────────────────
ss << LR"HTML(
...
// ─────────────────────────────────────────────────────────────
// PART 12d · Document + Forms + Help dropdown menus
// ─────────────────────────────────────────────────────────────
ss << LR"HTML(
...
// ─────────────────────────────────────────────────────────────
// PART 14a · Ribbon wrap + Home panel (navigate + draw groups)
// ─────────────────────────────────────────────────────────────
ss << LR"HTML(
...
// ─────────────────────────────────────────────────────────────
// PART 14b · Home ribbon: Insert + Shapes groups
// ─────────────────────────────────────────────────────────────
ss << LR"HTML(
  <div class="ribbon-group">
    <div class="rg-row">
...
// ─────────────────────────────────────────────────────────────
// PART 14c · Home ribbon: Style + Zoom groups
// ─────────────────────────────────────────────────────────────
ss << LR"HTML(
  <div class="ribbon-group">
    <div class="rg-row">
...
// ─────────────────────────────────────────────────────────────
// PART 14d · Annotate ribbon panel
// ─────────────────────────────────────────────────────────────
ss << LR"HTML(
<div class="ribbon-panel" id="r-annotate">
  <div class="ribbon-group">
    <div class="rg-row"><div class="rbtn-sm" onclick="setTool('highlight');closeAllMenus()"><svg viewBox="0 0 24 24" width="13" height="13"><rect x="3" y="14" width="18" height="5" rx="1" fill="#f9a825"/><path d="M6 13l6-9 6 9" fill="none" stroke="currentColor" stroke-width="1.5"/></svg>Highlight</div></div>
  ...
  <div class="ribbon-group">
    <div class="rg-row"><div class="rbtn-sm" onclick="setTool('pen')"><svg viewBox="0 0 24 24" width="13" height="13"><path d="M3 17.25V21h3.75L17.81 9.94l-3.75-3.75L3 17.25zm17.71-10.21a1 1 0 0 0 0-1.41l-2.34-2.34a1 1 0 0 0-1.41 0l-1.83 1.83 3.75 3.75 1.83-1.83z"/></svg>Free Draw</div></div>
  ...
  <div class="ribbon-group">
    <div class="rg-row"><div class="rbtn-sm" onclick="addBookmark()"><svg viewBox="0 0 24 24" width="13" height="13"><path d="M17 3H7a2 2 0 0 0-2 2v16l7-3 7 3V5a2 2 0 0 0-2-2z"/></svg>Add Bookmark</div></div>
...
// ─────────────────────────────────────────────────────────────
// PART 14e · Edit PDF + Pages + Protect + Export ribbon panels
// ─────────────────────────────────────────────────────────────
ss << LR"HTML(
<div class="ribbon-panel" id="r-edit">
  <div class="ribbon-group">
    <div class="rg-row"><div class="rbtn-sm" onclick="setTool('textbox')"><svg viewBox="0 0 24 24" width="13" height="13"><path d="M2 4v3h5v12h3V7h5V4H2zm19 5h-9v3h3v7h3v-7h3V9z"/></svg>Add Text</div></div>
...
<div class="ribbon-panel" id="r-pages">
  <div class="ribbon-group">
    <div class="rg-row"><div class="rbtn-sm" onclick="rotatePDFAll()"><svg viewBox="0 0 24 24" width="13" height="13"><path d="M12 5V1L7 6l5 5V7a7 7 0 1 1-7 7h-2a9 9 0 1 0 9-9z"/></svg>Rotate All</div></div>
...
// ─────────────────────────────────────────────────────────────
// PART 15a · Workspace + Left panel HTML
// ─────────────────────────────────────────────────────────────
ss << LR"HTML(
<div class="workspace">
...
// ─────────────────────────────────────────────────────────────
// PART 15b · Right panel HTML (doc info + annotation style)
// ─────────────────────────────────────────────────────────────
ss << LR"HTML(
...
// ─────────────────────────────────────────────────────────────
// PART 15c · Right panel HTML (quick actions + stamp + stats)
// ─────────────────────────────────────────────────────────────
ss << LR"HTML(
    <div class="rp-section">
...
// ─────────────────────────────────────────────────────────────
// PART 16 · Popups, modals, overlays HTML
// ─────────────────────────────────────────────────────────────
ss << LR"HTML(
...
let g_tabs=[],g_activeId=null,g_tool='hand';
let g_color='#e53935',g_fillColor='transparent';
let g_lineWidth=2,g_opacity=1.0,g_fontSize=12,g_fontFamily='Arial',g_sigColor='#1a1a1a';
let g_showRuler=false,g_showGrid=false,g_colorTarget='stroke',g_colorSwatchId=null;
let g_viewMode=0,g_ctxPageIndex=-1;
let g_drawing=false,g_drawCtx=null,g_drawCanvas=null,g_currentStroke=null,g_shapeStart=null;
...
function showModal(title,html,wide=false){
  ...
  body.style.maxWidth=wide?'600px':'';
...
async function saveBytesToFile(blob,name,ext='pdf',mime='application/pdf'){
  try{
    ...
    }else{saveAs(blob,name);showToast('Downloaded: '+name,'success');}
...
function updateStatusBar(){
  ...
  document.getElementById('rp-rotate').textContent=t.rotation+'deg';
...
ss << LR"JS(
<script>
function openFileDialog(multi=false){const fi=document.getElementById('fileInput');fi.multiple=multi;fi.click();}
async function handleFiles(e){
  for(const f of e.target.files){if(!f.name.toLowerCase().endsWith('.pdf'))continue;
    const bytes=new Uint8Array(await f.arrayBuffer());await createTab(f.name,bytes);}
  e.target.value='';
}
async function loadPdfFromPath(path){
  try{const url='file:///'+path.replace(/\\/g,'/');
    const res=await fetch(url);if(!res.ok)throw new Error('fetch failed');
    const bytes=new Uint8Array(await res.arrayBuffer());
    const name=path.split(/[\\/]/).pop();await createTab(name,bytes);
...
async function createTab(name,bytes){
  try{
    ...
    const tab={id,name,bytes:bytes.slice(),doc,zoom:1.0,rotation:0,
      annotations:[],pasteImages:[],textBoxes:[],redactions:[],bookmarks:[],
      pageOrder:Array.from({length:doc.numPages},(_,i)=>i+1),
      history:[],histIdx:-1,modified:false};
    g_tabs.push(tab);renderTabStrip();await switchTab(id);
  }catch(e){showToast('Invalid or corrupted PDF.');}
}
function renderTabStrip(){
  ...
  g_tabs.forEach(t=>{
    ...
    el.innerHTML='<span class="tab-icon">&#128196;</span>'+
    ...
    el.onclick=()=>switchTab(t.id);strip.appendChild(el);
  });
  const add=document.createElement('div');add.className='tab-add';add.textContent='+';
  add.onclick=()=>openFileDialog(true);strip.appendChild(add);
}
async function switchTab(id){
  g_activeId=id;renderTabStrip();const t=activeTab();if(!t)return;
  updateStatusBar();await renderViewer();buildThumbs();renderBookmarkPanel();refreshStats();
}
function closeActiveTab(){if(g_activeId)closeTab(g_activeId);}
function closeTab(id){
  g_tabs=g_tabs.filter(t=>t.id!==id);
  if(g_activeId===id)g_activeId=g_tabs.length>0?g_tabs[g_tabs.length-1].id:null;
  renderTabStrip();
...
async function renderViewer(){
  ...
  for(let i=0;i<t.pageOrder.length;i++){
    ...
    wrapper.className='page-wrapper';wrapper.id='pw-'+i;
    wrapper.dataset.pageIndex=i;wrapper.dataset.pdfPage=pNum;
    wrapper.style.width=vp.width+'px';wrapper.style.height=vp.height+'px';
    const pdfCanvas=document.createElement('canvas');
    pdfCanvas.className='pdf-canvas';pdfCanvas.width=vp.width;pdfCanvas.height=vp.height;
    const drawCanvas=document.createElement('canvas');
    drawCanvas.className='draw-canvas';drawCanvas.width=vp.width;drawCanvas.height=vp.height;
    ...
    const selRect=document.createElement('div');selRect.className='sel-rect';
    wrapper.appendChild(pdfCanvas);wrapper.appendChild(drawCanvas);
    wrapper.appendChild(grid);wrapper.appendChild(selRect);container.appendChild(wrapper);
    page.render({canvasContext:pdfCanvas.getContext('2d'),viewport:vp});
    restoreDrawAnnotations(i,drawCanvas,vp.width,vp.height);
    restoreShapes(t,i,wrapper);restoreTextBoxes(t,i,wrapper);
    restorePasteImages(t,i,wrapper);restoreStickyNotes(t,i,wrapper);
    restoreStamps(t,i,wrapper);restoreRedactions(t,i,wrapper);
  }
  updateStatusBar();refreshStats();
}
</script>
)JS";
...
ss << LR"JS(
<script>
function zoomBy(delta){const t=activeTab();if(!t)return;t.zoom=Math.min(5,Math.max(0.1,t.zoom+delta));updateStatusBar();scheduleRender();}
function zoomTo(v){const t=activeTab();if(!t)return;t.zoom=v;updateStatusBar();scheduleRender();}
function rotatePDFAll(){const t=activeTab();if(!t)return;t.rotation=(t.rotation+90)%360;updateStatusBar();scheduleRender();}
...
document.getElementById('viewer-area').addEventListener('mousemove',e=>{
  ...
  document.getElementById('sb-coords').textContent=Math.round(e.clientX-rect.left)+', '+Math.round(e.clientY-rect.top);
});
document.addEventListener('keydown',e=>{
  ...
  if(e.key==='1')setTool('hand');if(e.key==='2')setTool('select');
  if(e.key==='3')setTool('pen');if(e.key==='4')setTool('highlight');
  if(e.key==='5')setTool('eraser');if(e.key==='6')setTool('note');
  if(e.key==='7')setTool('textbox');if(e.key==='8')setTool('stamp');
  if(e.key==='r'||e.key==='R')rotatePDFAll();
});
let g_selectedAnnot=null;
function deleteSelectedAnnotation(){
  if(!g_selectedAnnot)return;const t=activeTab();if(!t)return;
  t.annotations=t.annotations.filter(a=>a!==g_selectedAnnot);
  g_selectedAnnot=null;scheduleRender();
}
</script>
)JS";
...
ss << LR"JS(
<script>
const TOOL_LABELS={hand:'Hand',select:'Select',pen:'Pen',highlight:'Highlight',
...
function setTool(tool){
  ...
  const rbtn=document.getElementById('rb-'+tool);if(rbtn)rbtn.classList.add('active');
...
viewerArea.addEventListener('mousedown',e=>{
  ...
  g_panning=true;g_panX0=e.clientX;g_panY0=e.clientY;
  g_scrollX0=viewerArea.scrollLeft;g_scrollY0=viewerArea.scrollTop;
...
// ─────────────────────────────────────────────────────────────
// PART 22 · JS: Canvas drawing (pen, highlight, eraser)
// ─────────────────────────────────────────────────────────────
ss << LR"JS(
<script>
function getCanvasXY(canvas,e){const r=canvas.getBoundingClientRect();return{x:e.clientX-r.left,y:e.clientY-r.top};}
document.getElementById('pdf-container').addEventListener('mousedown',e=>{
  ...
  g_drawing=true;g_drawCanvas=canvas;g_drawCtx=canvas.getContext('2d');
  ...
  g_currentStroke={type:'stroke',tool:g_tool,pageIndex:pIdx,
    color:col,lineWidth:lw,alpha,
    points:[{rx:pos.x/canvas.width,ry:pos.y/canvas.height}],id:'a_'+Date.now()};
  g_drawCtx.save();g_drawCtx.strokeStyle=col;g_drawCtx.lineWidth=lw;
  g_drawCtx.lineCap='round';g_drawCtx.lineJoin='round';g_drawCtx.globalAlpha=alpha;
  g_drawCtx.beginPath();g_drawCtx.moveTo(pos.x,pos.y);e.preventDefault();
});
window.addEventListener('mousemove',e=>{
  if(!g_drawing||!g_drawCtx||!g_drawCanvas)return;
  const pos=getCanvasXY(g_drawCanvas,e);
  g_drawCtx.lineTo(pos.x,pos.y);g_drawCtx.stroke();
  g_drawCtx.beginPath();g_drawCtx.moveTo(pos.x,pos.y);
  if(g_currentStroke)g_currentStroke.points.push({rx:pos.x/g_drawCanvas.width,ry:pos.y/g_drawCanvas.height});
});
window.addEventListener('mouseup',()=>{
  if(!g_drawing)return;g_drawing=false;
  if(g_drawCtx){g_drawCtx.restore();g_drawCtx=null;}
  const t=activeTab();
  if(t&&g_currentStroke&&g_currentStroke.points.length>1){
    pushHistory(t);t.annotations.push(g_currentStroke);t.modified=true;renderTabStrip();refreshStats();
  }
...
function restoreDrawAnnotations(pageIndex,canvas,w,h){
  ...
  t.annotations.filter(a=>a.type==='stroke'&&a.pageIndex===pageIndex).forEach(a=>{
    ...
    ctx.save();ctx.strokeStyle=a.color;ctx.lineWidth=a.lineWidth;
...
function eraseAt(canvas,e){
  const pos=getCanvasXY(canvas,e);const ctx=canvas.getContext('2d');
  const r=20*(g_lineWidth/2);
  ctx.save();ctx.globalCompositeOperation='destination-out';
  ctx.beginPath();ctx.arc(pos.x,pos.y,r,0,Math.PI*2);ctx.fill();ctx.restore();
  const t=activeTab();if(!t)return;
  const wrapper=canvas.parentElement;const pIdx=parseInt(wrapper.dataset.pageIndex);
  const cw=canvas.width,ch=canvas.height;
  t.annotations=t.annotations.filter(a=>{
    if(a.type!=='stroke'||a.pageIndex!==pIdx)return true;
    return !a.points.some(p=>Math.hypot(p.rx*cw-pos.x,p.ry*ch-pos.y)<r);
  });
}
</script>
)JS";
...
// ─────────────────────────────────────────────────────────────
// PART 23 · JS: Shape drawing
// ─────────────────────────────────────────────────────────────
ss << LR"JS(
<script>
let g_shapeCanvas=null,g_shapeCtx=null,g_shapeDrawing=false;
function startShape(canvas,e){
  g_shapeCanvas=canvas;g_shapeCtx=canvas.getContext('2d');
  g_shapeStart=getCanvasXY(canvas,e);g_shapeDrawing=true;e.preventDefault();
}
window.addEventListener('mousemove',e=>{
  ...
  ctx.clearRect(0,0,w,h);restoreDrawAnnotations(pIdx,g_shapeCanvas,w,h);
  ctx.save();ctx.strokeStyle=g_color;ctx.lineWidth=g_lineWidth;ctx.globalAlpha=g_opacity;
  const x=Math.min(g_shapeStart.x,pos.x),y=Math.min(g_shapeStart.y,pos.y);
  const w2=Math.abs(pos.x-g_shapeStart.x),h2=Math.abs(pos.y-g_shapeStart.y);
  ctx.beginPath();
  if(g_tool==='rect'){ctx.rect(x,y,w2,h2);}
  else if(g_tool==='ellipse'){ctx.ellipse(x+w2/2,y+h2/2,w2/2,h2/2,0,0,Math.PI*2);}
  else if(g_tool==='line'){ctx.moveTo(g_shapeStart.x,g_shapeStart.y);ctx.lineTo(pos.x,pos.y);}
  else if(g_tool==='arrow'){
    ctx.moveTo(g_shapeStart.x,g_shapeStart.y);ctx.lineTo(pos.x,pos.y);
    const ang=Math.atan2(pos.y-g_shapeStart.y,pos.x-g_shapeStart.x),al=12;
    ctx.moveTo(pos.x,pos.y);ctx.lineTo(pos.x-al*Math.cos(ang-0.4),pos.y-al*Math.sin(ang-0.4));
    ctx.moveTo(pos.x,pos.y);ctx.lineTo(pos.x-al*Math.cos(ang+0.4),pos.y-al*Math.sin(ang+0.4));
  }else if(g_tool==='redact'){ctx.fillStyle='rgba(0,0,0,0.5)';ctx.fillRect(x,y,w2,h2);}
  if(g_fillColor!=='transparent'&&['rect','ellipse'].includes(g_tool)){ctx.fillStyle=g_fillColor;ctx.fill();}
  ctx.stroke();ctx.restore();
});
window.addEventListener('mouseup',e=>{
  if(!g_shapeDrawing)return;g_shapeDrawing=false;
  const t=activeTab();if(!t||!g_shapeCanvas){g_shapeCanvas=null;return;}
  const pos=getCanvasXY(g_shapeCanvas,e);
  const wrapper=g_shapeCanvas.parentElement;
  const pIdx=parseInt(wrapper.dataset.pageIndex);
  const w=g_shapeCanvas.width,h=g_shapeCanvas.height;
  if(Math.abs(pos.x-g_shapeStart.x)<3&&Math.abs(pos.y-g_shapeStart.y)<3){g_shapeCanvas=null;return;}
  pushHistory(t);
  t.annotations.push({type:'shape',tool:g_tool,pageIndex:pIdx,
    x1:g_shapeStart.x/w,y1:g_shapeStart.y/h,x2:pos.x/w,y2:pos.y/h,
    color:g_color,fillColor:g_fillColor,lineWidth:g_lineWidth,alpha:g_opacity,id:'sh_'+Date.now()});
  t.modified=true;renderTabStrip();g_shapeCanvas=null;g_shapeStart=null;
});
function restoreShapes(tab,pageIndex,wrapper){
  const canvas=wrapper.querySelector('.draw-canvas');if(!canvas)return;
  ...
  tab.annotations.filter(a=>a.type==='shape'&&a.pageIndex===pageIndex).forEach(a=>{
    ...
    ctx.save();ctx.strokeStyle=a.color;ctx.lineWidth=a.lineWidth;ctx.globalAlpha=a.alpha||1;
    ctx.beginPath();
    ...
    if(a.fillColor&&a.fillColor!=='transparent'&&['rect','ellipse'].includes(a.tool)){ctx.fillStyle=a.fillColor;ctx.fill();}
    ctx.stroke();ctx.restore();
  });
}
</script>
...
function restoreStickyNotes(tab,pageIndex,wrapper){
  tab.annotations.filter(a=>a.type==='note'&&a.pageIndex===pageIndex).forEach(a=>createNoteEl(a,wrapper));
}
function createNoteEl(data,wrapper){
  const note=document.createElement('div');
  note.style.cssText='position:absolute;z-index:10;background:'+(data.color||'#fff9c4')+
    ';border:1px solid #d4b800;border-radius:3px;padding:5px 6px;font-size:11px;'+
    'min-width:90px;min-height:44px;max-width:200px;box-shadow:2px 3px 8px rgba(0,0,0,.2);'+
    'cursor:move;resize:both;overflow:auto;color:#333;white-space:pre-wrap;word-break:break-word;'+
    'left:'+(data.rx*100)+'%;top:'+(data.ry*100)+'%;';
  note.contentEditable='true';note.textContent=data.text||'';
  const bar=document.createElement('div');bar.style.cssText='display:flex;gap:3px;margin-bottom:3px;';
  NOTE_COLORS.forEach(c=>{
    ...
    sw.onclick=()=>{note.style.background=c;data.color=c;};bar.appendChild(sw);
  });
  const close=document.createElement('span');close.textContent='x';
  close.style.cssText='margin-left:auto;cursor:pointer;color:#999;font-size:12px;line-height:1;';
  close.onclick=()=>{const t=activeTab();if(t)t.annotations=t.annotations.filter(a=>a.id!==data.id);note.remove();refreshStats();};
  bar.appendChild(close);note.prepend(bar);
  note.addEventListener('input',()=>{data.text=note.textContent;});
  makeDraggable(note,wrapper,data);wrapper.appendChild(note);
}
...
function makeDraggable(el,container,data){
  ...
  const onDown=e=>{
    ...
    drag=true;sx=e.clientX;sy=e.clientY;ex=el.offsetLeft;ey=el.offsetTop;e.preventDefault();
  };
  const onMove=e=>{
    if(!drag)return;
...
function restoreTextBoxes(tab,pageIndex,wrapper){
  tab.textBoxes.filter(tb=>tb.pageIndex===pageIndex).forEach(tb=>createTextBoxEl(tb,wrapper));
}
function createTextBoxEl(data,wrapper){
  const tb=document.createElement('div');tb.className='textbox-el';
  tb.style.left=(data.rx*100)+'%';tb.style.top=(data.ry*100)+'%';
  tb.style.fontSize=(data.fontSize||12)+'px';tb.style.fontFamily=data.fontFamily||'Arial';
  tb.style.color=data.color||'#111';tb.contentEditable='true';tb.textContent=data.text||'';
  const close=document.createElement('span');close.textContent='x';
  close.style.cssText='position:absolute;top:1px;right:3px;cursor:pointer;font-size:11px;color:#aaa;';
  close.onclick=()=>{const t=activeTab();if(t)t.textBoxes=t.textBoxes.filter(x=>x!==data);tb.remove();};
  tb.appendChild(close);tb.addEventListener('input',()=>{data.text=tb.textContent;});
  makeDraggable(tb,wrapper,data);wrapper.appendChild(tb);
}
document.getElementById('pdf-container').addEventListener('click',e=>{
  ...
  const rect=wrapper.getBoundingClientRect();
  const data={pageIndex:parseInt(wrapper.dataset.pageIndex),
    rx:(e.clientX-rect.left)/rect.width,ry:(e.clientY-rect.top)/rect.height,
...
ss << LR"JS(
<script>
const STAMP_COLORS={DRAFT:'#1565c0',APPROVED:'#2e7d32',REJECTED:'#c62828',
  CONFIDENTIAL:'#6a1b9a',FINAL:'#00695c',VOID:'#4e342e',COPY:'#616161',RECEIVED:'#1b5e20'};
function restoreStamps(tab,pageIndex,wrapper){
  tab.annotations.filter(a=>a.type==='stamp'&&a.pageIndex===pageIndex).forEach(a=>createStampEl(a,wrapper));
}
function createStampEl(data,wrapper){
  const el=document.createElement('div');el.className='stamp-el';
  ...
  del.style.cssText='position:absolute;top:-6px;right:-6px;background:red;color:#fff;border-radius:50%;width:14px;height:14px;font-size:10px;display:flex;align-items:center;justify-content:center;cursor:pointer;';
  del.onclick=()=>{const t=activeTab();if(t)t.annotations=t.annotations.filter(a=>a!==data);el.remove();};
  el.appendChild(del);makeDraggable(el,wrapper,data);wrapper.appendChild(el);
}
document.getElementById('pdf-container').addEventListener('click',e=>{
  ...
  const data={type:'stamp',id:'st_'+Date.now(),pageIndex:parseInt(wrapper.dataset.pageIndex),
    rx:(e.clientX-rect.left)/rect.width,ry:(e.clientY-rect.top)/rect.height,label};
  const t=activeTab();if(!t)return;
  pushHistory(t);t.annotations.push(data);t.modified=true;createStampEl(data,wrapper);renderTabStrip();
});
function restorePasteImages(tab,pageIndex,wrapper){
  tab.pasteImages.filter(img=>img.pageIndex===pageIndex).forEach(img=>createImgOverlay(img,wrapper));
}
function createImgOverlay(data,wrapper){
  ...
  const img=document.createElement('img');img.src=data.src;img.style.cssText='width:100%;height:100%;display:block;pointer-events:none;';
  const del=document.createElement('span');del.textContent='x';
  ...
  del.onclick=()=>{const t=activeTab();if(t)t.pasteImages=t.pasteImages.filter(i=>i!==data);div.remove();};
  ...
  div.appendChild(rh);makeDraggable(div,wrapper,null);
  div.addEventListener('mouseup',()=>{data.rx=div.offsetLeft/wrapper.offsetWidth;data.ry=div.offsetTop/wrapper.offsetHeight;});
  wrapper.appendChild(div);
}
function startImgResize(e,el,data,wrapper){
  e.stopPropagation();e.preventDefault();
  const sx=e.clientX,sy=e.clientY,sw=el.offsetWidth,sh=el.offsetHeight;
  const onM=ev=>{el.style.width=Math.max(40,sw+(ev.clientX-sx))+'px';el.style.height=Math.max(30,sh+(ev.clientY-sy))+'px';};
  const onU=()=>{data.rw=el.offsetWidth/wrapper.offsetWidth;data.rh=el.offsetHeight/wrapper.offsetHeight;
...
document.addEventListener('paste',async e=>{
  ...
  for(let i=0;i<e.clipboardData.items.length;i++){
    if(e.clipboardData.items[i].type.startsWith('image')){file=e.clipboardData.items[i].getAsFile();break;}
  }
  if(!file)return;
  const buf=new Uint8Array(await file.arrayBuffer());
  const src=URL.createObjectURL(file);
  const first=document.getElementById('pdf-container').querySelector('.page-wrapper');
  if(!first){showToast('Open a PDF first.');return;}
  const data={id:'img_'+Date.now(),pageIndex:parseInt(first.dataset.pageIndex)||0,
    rx:0.05,ry:0.05,rw:0.4,rh:0.3,src,buffer:buf,mimeType:file.type};
  pushHistory(t);t.pasteImages.push(data);t.modified=true;
  createImgOverlay(data,first);renderTabStrip();showToast('Image pasted.','success');
});
</script>
)JS";
...
function restoreRedactions(tab,pageIndex,wrapper){
  tab.redactions.filter(r=>r.pageIndex===pageIndex).forEach(r=>createRedactEl(r,wrapper));
}
function createRedactEl(data,wrapper){
  const el=document.createElement('div');el.className='redact-rect';
  el.style.left=(data.x*100)+'%';el.style.top=(data.y*100)+'%';
  el.style.width=(data.w*100)+'%';el.style.height=(data.h*100)+'%';
  el.title='Click to remove';
  el.onclick=()=>{const t=activeTab();if(!t)return;t.redactions=t.redactions.filter(r=>r!==data);el.remove();};
...
function applyRedactions(){
  ...
  scheduleRender();showToast('Redactions applied.','success');
}
</script>
)JS";

// ─────────────────────────────────────────────────────────────
// PART 28 · JS: Thumbnails + Bookmarks
// ─────────────────────────────────────────────────────────────
ss << LR"JS(
<script>
async function buildThumbs(){
  ...
  for(let i=0;i<t.pageOrder.length;i++){
    const pNum=t.pageOrder[i];const page=await t.doc.getPage(pNum);
    const vp=page.getViewport({scale:0.17});
    const item=document.createElement('div');
    item.className='thumb-item'+(i===0?' selected':'');item.dataset.orderIdx=i;
    const c=document.createElement('canvas');c.width=vp.width;c.height=vp.height;
    page.render({canvasContext:c.getContext('2d'),viewport:vp});
    const num=document.createElement('div');num.className='thumb-pg-num';num.textContent=i+1;
    const del=document.createElement('div');del.className='thumb-del';del.textContent='x';
    ...
    item.onclick=()=>{
      ...
      item.classList.add('selected');document.getElementById('pw-'+i)?.scrollIntoView({block:'start',behavior:'smooth'});
    };
    setupThumbDnd(item,list,i);list.appendChild(item);
  }
}
function deletePageByOrder(idx){
  const t=activeTab();if(!t)return;
  if(t.pageOrder.length<=1){showToast('Cannot delete the only page.');return;}
  pushHistory(t);t.pageOrder.splice(idx,1);
  ['annotations','textBoxes','pasteImages','redactions'].forEach(key=>{
    t[key]=t[key].filter(a=>a.pageIndex!==idx).map(a=>({...a,pageIndex:a.pageIndex>idx?a.pageIndex-1:a.pageIndex}));
  });
  t.modified=true;renderTabStrip();renderViewer();buildThumbs();showToast('Page deleted.');
}
function setupThumbDnd(item,list,startIdx){
  item.draggable=true;
  item.addEventListener('dragstart',e=>{e.dataTransfer.setData('text/plain',startIdx);item.style.opacity='.4';});
  ...
  item.addEventListener('drop',e=>{
    ...
    const from=parseInt(e.dataTransfer.getData('text/plain')),to=startIdx;
    if(from===to)return;const t=activeTab();if(!t)return;
    const moved=t.pageOrder.splice(from,1)[0];t.pageOrder.splice(to,0,moved);
    t.modified=true;renderViewer();buildThumbs();
  });
}
function switchLPanel(which,btn){
  ['thumb','bm','layers'].forEach(id=>{document.getElementById('lp-'+id).style.display=id===which?'':'none';});
  document.querySelectorAll('.lp-tab').forEach(b=>b.classList.remove('active'));
  if(btn)btn.classList.add('active');
  if(which==='thumb')buildThumbs();if(which==='bm')renderBookmarkPanel();
}
function addBookmark(){
  const t=activeTab();if(!t){showToast('No file open.');return;}
  const label=prompt('Bookmark label:','Page 1');if(!label)return;
  t.bookmarks.push({pageIndex:0,label});renderBookmarkPanel();showToast('Bookmark added.','success');
}
function addBookmarkAtCtx(){
  const t=activeTab();if(!t)return;
  const label=prompt('Bookmark label:','Bookmark');if(!label)return;
  t.bookmarks.push({pageIndex:Math.max(0,g_ctxPageIndex),label});renderBookmarkPanel();closeCtx();showToast('Bookmark added.','success');
}
function renderBookmarkPanel(){
  const t=activeTab();const panel=document.getElementById('lp-bm');
  if(!t||!t.bookmarks.length){panel.innerHTML='<div style="padding:10px;font-size:10.5px;color:var(--c-muted);">No bookmarks.</div>';return;}
  panel.innerHTML='';
  t.bookmarks.forEach((bm,i)=>{
    const el=document.createElement('div');el.className='bm-item';
    el.innerHTML='&#9733; '+bm.label+'<span class="bm-del" onclick="deleteBookmark('+i+')">&#128465;</span>';
    el.onclick=()=>{document.getElementById('pw-'+bm.pageIndex)?.scrollIntoView({block:'start',behavior:'smooth'});};
    panel.appendChild(el);
  });
}
function deleteBookmark(i){const t=activeTab();if(!t)return;t.bookmarks.splice(i,1);renderBookmarkPanel();}
</script>
)JS";
...
function openSignatureModal(){
  ...
  const c=document.getElementById('sig-modal-canvas');g_sigCtx=c.getContext('2d');
  g_sigCtx.fillStyle='#fff';g_sigCtx.fillRect(0,0,c.width,c.height);
}
function closeSigModal(){document.getElementById('sig-modal-overlay').classList.remove('show');}
function clearSigPad(){const c=document.getElementById('sig-modal-canvas');g_sigCtx.fillStyle='#fff';g_sigCtx.fillRect(0,0,c.width,c.height);}
const sigCanvas=document.getElementById('sig-modal-canvas');
sigCanvas.addEventListener('mousedown',e=>{
  g_sigDrawing=true;const r=sigCanvas.getBoundingClientRect();
  g_sigCtx.beginPath();g_sigCtx.moveTo(e.clientX-r.left,e.clientY-r.top);
  g_sigCtx.strokeStyle=g_sigColor;g_sigCtx.lineWidth=2;g_sigCtx.lineCap='round';
});
sigCanvas.addEventListener('mousemove',e=>{
  if(!g_sigDrawing)return;const r=sigCanvas.getBoundingClientRect();
  g_sigCtx.lineTo(e.clientX-r.left,e.clientY-r.top);g_sigCtx.stroke();
  g_sigCtx.beginPath();g_sigCtx.moveTo(e.clientX-r.left,e.clientY-r.top);
});
...
function applySigToPage(){
  ...
  const data={id:'sig_'+Date.now(),pageIndex:parseInt(first.dataset.pageIndex)||0,
    rx:0.05,ry:0.7,rw:0.35,rh:0.1,src,mimeType:'image/png'};
  pushHistory(t);t.pasteImages.push(data);t.modified=true;
  createImgOverlay(data,first);closeSigModal();renderTabStrip();showToast('Signature inserted.','success');
}
</script>
)JS";

// ─────────────────────────────────────────────────────────────
// PART 30a · JS: Colour picker
// ─────────────────────────────────────────────────────────────
ss << LR"JS(
<script>
const PALETTE=['#000000','#434343','#666666','#999999','#b7b7b7','#cccccc','#d9d9d9','#ffffff',
...
function showColorPicker(target,swatchId){
  ...
  PALETTE.forEach(c=>{
    const cell=document.createElement('div');cell.className='color-cell';cell.style.background=c;
    cell.onclick=()=>{applyCustomColor(c);};grid.appendChild(cell);
  });
  const popup=document.getElementById('color-picker-popup');
  const swatch=document.getElementById(swatchId);
  if(swatch){const r=swatch.getBoundingClientRect();popup.style.left=r.left+'px';popup.style.top=(r.bottom+4)+'px';}
  popup.classList.add('show');
}
function applyCustomColor(c){
  if(g_colorTarget==='stroke'){
    g_color=c;if(g_colorSwatchId)document.getElementById(g_colorSwatchId).style.background=c;
    document.getElementById('rp-color').style.background=c;document.getElementById('rb-color-stroke').style.background=c;
  }else{
    g_fillColor=c;if(g_colorSwatchId)document.getElementById(g_colorSwatchId).style.background=c;
...
document.addEventListener('click',e=>{
  ...
  if(pp.classList.contains('show')&&!pp.contains(e.target)&&!e.target.classList.contains('color-swatch'))pp.classList.remove('show');
});
</script>
)JS";

// ─────────────────────────────────────────────────────────────
// PART 30b · JS: Undo/Redo + Menus
// ─────────────────────────────────────────────────────────────
ss << LR"JS(
<script>
function pushHistory(t){
  const snap={annotations:JSON.parse(JSON.stringify(t.annotations)),
    textBoxes:JSON.parse(JSON.stringify(t.textBoxes)),
    redactions:JSON.parse(JSON.stringify(t.redactions)),pageOrder:[...t.pageOrder]};
  t.history.splice(t.histIdx+1);t.history.push(snap);
  if(t.history.length>MAX_HIST)t.history.shift();t.histIdx=t.history.length-1;
}
function histUndo(){
  const t=activeTab();if(!t||t.histIdx<0){showToast('Nothing to undo.');return;}
  t.histIdx--;
  if(t.histIdx<0){t.annotations=[];t.textBoxes=[];t.redactions=[];}
  else{const snap=t.history[t.histIdx];
...
function histRedo(){
  ...
  t.histIdx++;const snap=t.history[t.histIdx];
  t.annotations=JSON.parse(JSON.stringify(snap.annotations));
  t.textBoxes=JSON.parse(JSON.stringify(snap.textBoxes));
  t.redactions=JSON.parse(JSON.stringify(snap.redactions));
  t.pageOrder=[...snap.pageOrder];scheduleRender();buildThumbs();showToast('Redo.');
}
function toggleMenu(id,btn){
  const m=document.getElementById(id);const isOpen=m.classList.contains('show');
  closeAllMenus();
  if(!isOpen){const r=btn.getBoundingClientRect();m.style.left=r.left+'px';m.style.top=r.bottom+'px';
...
document.addEventListener('click',e=>{if(!e.target.closest('.dropdown')&&!e.target.closest('.top-menu'))closeAllMenus();});
...
function toggleRPSection(hdr){
  const body=hdr.nextElementSibling;const collapsed=body.style.display==='none';
  body.style.display=collapsed?'':'none';hdr.querySelector('.toggle').textContent=collapsed?'v':'>';
}
document.getElementById('pdf-container').addEventListener('contextmenu',e=>{
  e.preventDefault();const wrapper=e.target.closest('.page-wrapper');
  g_ctxPageIndex=wrapper?parseInt(wrapper.dataset.pageIndex):-1;
  const m=document.getElementById('ctx-menu');m.style.left=e.clientX+'px';m.style.top=e.clientY+'px';
...
function ctxDeletePage(){if(g_ctxPageIndex>=0)deletePageByOrder(g_ctxPageIndex);closeCtx();}
function copySelectedText(){const sel=window.getSelection();if(sel){document.execCommand('copy');showToast('Copied.','success');}}
</script>
)JS";
...
function toggleFindBar(){
  const fb=document.getElementById('findbar');fb.classList.toggle('show');
...
async function doFind(){
  ...
  for(let i=0;i<t.pageOrder.length;i++){
    ...
    let pos=0;while((pos=text.indexOf(q,pos))!==-1){g_findMatches.push({pageIndex:i,charPos:pos});pos+=q.length;}
  }
  document.getElementById('find-count').textContent=g_findMatches.length>0?'1 of '+g_findMatches.length:'Not found';
  if(g_findMatches.length){const m=g_findMatches[0];document.getElementById('pw-'+m.pageIndex)?.scrollIntoView({block:'start',behavior:'smooth'});}
}
function findNext(){
  if(!g_findMatches.length)return;g_findIdx=(g_findIdx+1)%g_findMatches.length;
  document.getElementById('find-count').textContent=(g_findIdx+1)+' of '+g_findMatches.length;
  document.getElementById('pw-'+g_findMatches[g_findIdx].pageIndex)?.scrollIntoView({block:'start',behavior:'smooth'});
}
function findPrev(){
  if(!g_findMatches.length)return;g_findIdx=(g_findIdx-1+g_findMatches.length)%g_findMatches.length;
  document.getElementById('find-count').textContent=(g_findIdx+1)+' of '+g_findMatches.length;
  document.getElementById('pw-'+g_findMatches[g_findIdx].pageIndex)?.scrollIntoView({block:'start',behavior:'smooth'});
}
function clearHighlights(){g_findMatches=[];g_findIdx=0;document.getElementById('find-count').textContent='';}
</script>
)JS";
...
async function downloadCurrentPDF(saveAs2=false){
  ...
  try{
    ...
    copied.forEach(p=>out.addPage(p));const pages=out.getPages();setProgress(30);
    for(const a of t.annotations){
      ...
      if(a.type==='stroke'&&a.points&&a.points.length>=2){
        ...
        for(let i=1;i<a.points.length;i++){
          page.drawLine({start:{x:a.points[i-1].rx*width,y:height-(a.points[i-1].ry*height)},
            end:{x:a.points[i].rx*width,y:height-(a.points[i].ry*height)},
            thickness:a.lineWidth,color:col,opacity:a.alpha||1});
        }
      }else if(a.type==='shape'){
        const x1=a.x1*width,y1=height-(a.y1*height),x2=a.x2*width,y2=height-(a.y2*height);
        const col=hexToRgb(a.color);
        if(a.tool==='rect'){const rx=Math.min(x1,x2),ry=Math.min(y1,y2);
          page.drawRectangle({x:rx,y:ry,width:Math.abs(x2-x1),height:Math.abs(y2-y1),borderColor:col,borderWidth:a.lineWidth,opacity:a.alpha||1});}
        else if(a.tool==='line'||a.tool==='arrow'){page.drawLine({start:{x:x1,y:y1},end:{x:x2,y:y2},thickness:a.lineWidth,color:col,opacity:a.alpha||1});}
        else if(a.tool==='redact'){const rx=Math.min(x1,x2),ry=Math.min(y1,y2);page.drawRectangle({x:rx,y:ry,width:Math.abs(x2-x1),height:Math.abs(y2-y1),color:PDFLib.rgb(0,0,0)});}
      }else if(a.type==='note'){
        const nx=a.rx*width,ny=height-(a.ry*height);
        page.drawRectangle({x:nx,y:ny-32,width:160,height:32,color:PDFLib.rgb(1,.97,.77),borderColor:PDFLib.rgb(.8,.75,.1),borderWidth:1,opacity:.9});
        page.drawText(a.text||'',{x:nx+4,y:ny-24,size:8,color:PDFLib.rgb(.1,.1,.1),maxWidth:150});
      }else if(a.type==='stamp'){
        const col=hexToRgb(STAMP_COLORS[a.label]||'#555');
        page.drawText(a.label,{x:a.rx*width,y:height-(a.ry*height),size:24,color:col,opacity:.5});
      }
    }
    setProgress(60);
    for(const r of t.redactions){
      if(r.pageIndex>=pages.length)continue;
      const page=pages[r.pageIndex];const {width,height}=page.getSize();
      page.drawRectangle({x:r.x*width,y:height-(r.y*height)-(r.h*height),width:r.w*width,height:r.h*height,color:PDFLib.rgb(0,0,0)});
    }
    for(const tb of t.textBoxes){
      if(tb.pageIndex>=pages.length)continue;
      const page=pages[tb.pageIndex];const {width,height}=page.getSize();
      page.drawText(tb.text||'',{x:tb.rx*width,y:height-(tb.ry*height),size:tb.fontSize||12,color:hexToRgb(tb.color||'#000')});
    }
    for(const img of t.pasteImages){
      if(img.pageIndex>=pages.length)continue;
      const page=pages[img.pageIndex];const {width,height}=page.getSize();
      try{let emb;if(img.mimeType==='image/png')emb=await out.embedPng(img.buffer);else emb=await out.embedJpg(img.buffer);
        const iw=img.rw*width,ih=img.rh*height;
        page.drawImage(emb,{x:img.rx*width,y:height-(img.ry*height)-ih,width:iw,height:ih});}catch(e){}
    }
    setProgress(90);
    await saveBytesToFile(new Blob([await out.save()],{type:'application/pdf'}),saveAs2?undefined:t.name||t.name);
...
ss << LR"JS(
<script>
function actionMergePDFs(){document.getElementById('mergeInput').click();}
async function doMerge(e){
  ...
  try{
    const out=await PDFLib.PDFDocument.create();
    for(let i=0;i<files.length;i++){setProgress(Math.round(i/files.length*80));
      const bytes=new Uint8Array(await files[i].arrayBuffer());
      const src=await PDFLib.PDFDocument.load(bytes);
      const copied=await out.copyPages(src,src.getPageIndices());
      copied.forEach(p=>out.addPage(p));}
    setProgress(95);await saveBytesToFile(new Blob([await out.save()]),'Combined.pdf');
  }catch(e){showToast('Merge failed: '+e.message);}
  showLoading(false);
}
function modalSplit(){
  const t=activeTab();if(!t){showToast('Open a PDF.');return;}
  showModal('Split PDF','<label>Split after page (1-'+(t.pageOrder.length-1)+'):</label>'+
...
function modalExtract(){
  const t=activeTab();if(!t){showToast('Open a PDF.');return;}
  showModal('Extract Pages','<label>Pages to extract (e.g. 1, 3, 5-7):</label>'+
    '<input type="text" id="extract-pages" placeholder="1, 2, 3">'+
    '<div class="modal-actions"><button class="btn btn-secondary" onclick="closeModal()">Cancel</button>'+
    '<button class="btn btn-primary" onclick="doExtract()">Extract</button></div>');
}
async function doExtract(){
  const t=activeTab();if(!t)return;
  const idxs=parsePageRange(document.getElementById('extract-pages').value,t.pageOrder.length);
  if(!idxs.length){showToast('No valid pages.');return;}
  closeModal();showLoading(true,'Extracting...',10);
  try{
    const src=await PDFLib.PDFDocument.load(t.bytes);const out=await PDFLib.PDFDocument.create();
    (await out.copyPages(src,idxs.map(i=>t.pageOrder[i]-1))).forEach(p=>out.addPage(p));
    setProgress(80);await saveBytesToFile(new Blob([await out.save()]),'Extracted.pdf');
  }catch(e){showToast('Extract failed.');}showLoading(false);
}
function parsePageRange(str,total){
  const result=[];
  str.split(',').forEach(part=>{
    part=part.trim();const dash=part.match(/^(\d+)-(\d+)$/);
    if(dash){for(let i=parseInt(dash[1]);i<=parseInt(dash[2]);i++){const idx=i-1;if(idx>=0&&idx<total)result.push(idx);}}
    else{const n=parseInt(part)-1;if(!isNaN(n)&&n>=0&&n<total)result.push(n);}
  });
  return[...new Set(result)].sort((a,b)=>a-b);
}
function modalDeletePages(){
  const t=activeTab();if(!t){showToast('Open a PDF.');return;}
  showModal('Delete Pages','<label>Pages to delete (e.g. 2, 4, 6-8):</label>'+
    '<input type="text" id="del-pages" placeholder="e.g. 1, 3">'+
    '<div class="modal-actions"><button class="btn btn-secondary" onclick="closeModal()">Cancel</button>'+
    '<button class="btn btn-primary" style="background:#c62828;" onclick="doDeletePages()">Delete</button></div>');
}
function doDeletePages(){
  const t=activeTab();if(!t)return;
  const toRemove=new Set(parsePageRange(document.getElementById('del-pages').value,t.pageOrder.length));
  ...
  t.modified=true;renderViewer();buildThumbs();showToast('Page(s) deleted.','success');
}
function modalInsertBlank(){
  const t=activeTab();if(!t){showToast('Open a PDF.');return;}
  showModal('Insert Blank Page','<label>Insert after page (0=before first):</label>'+
    '<input type="number" id="insert-after" min="0" max="'+t.pageOrder.length+'" value="'+t.pageOrder.length+'">'+
    '<div class="modal-actions"><button class="btn btn-secondary" onclick="closeModal()">Cancel</button>'+
    '<button class="btn btn-blue" onclick="doInsertBlank()">Insert</button></div>');
}
async function doInsertBlank(){
  ...
  try{
    ...
    out.addPage([612,792]);
    const newOrder=[];
    copied.forEach((p,i)=>{if(i===after)newOrder.push(out.getPage(copied.length));newOrder.push(p);});
    setProgress(70);
    const saved=await out.save();const newBytes=new Uint8Array(saved);
    const newDoc=await pdfjsLib.getDocument({data:newBytes}).promise;
    t.bytes=newBytes;t.doc=newDoc;t.pageOrder=Array.from({length:newDoc.numPages},(_,i)=>i+1);
    t.modified=true;renderViewer();buildThumbs();renderTabStrip();showToast('Blank page inserted.','success');
  }catch(e){showToast('Insert failed: '+e.message);}showLoading(false);
}
</script>
)JS";
...
function modalWatermark(){
  ...
  showModal('Add Watermark',
    '<label>Text:</label><input type="text" id="wm-text" value="CONFIDENTIAL">'+
    '<label>Font size:</label><input type="number" id="wm-size" value="60" min="12" max="200">'+
    '<label>Opacity (1-100):</label><input type="number" id="wm-opacity" value="18" min="1" max="100">'+
    '<label>Colour:</label><input type="color" id="wm-color" value="#cc0000" style="width:48px;height:28px;border:none;">'+
    '<label>Angle:</label><input type="number" id="wm-angle" value="40" min="0" max="360">'+
    '<label>Apply to:</label><select id="wm-pages"><option value="all">All pages</option>'+
    '<option value="odd">Odd pages</option><option value="even">Even pages</option><option value="first">First page only</option></select>'+
    '<div class="modal-actions"><button class="btn btn-secondary" onclick="closeModal()">Cancel</button>'+
    '<button class="btn btn-primary" onclick="doWatermark()">Apply</button></div>');
}
async function doWatermark(){
  ...
  try{
    ...
    doc.getPages().forEach((page,i)=>{
      if(which==='odd'&&i%2!==0)return;if(which==='even'&&i%2===0)return;if(which==='first'&&i>0)return;
      const {width,height}=page.getSize();
      page.drawText(txt,{x:width/2-sz*txt.length/4,y:height/2,size:sz,color:col,opacity:op,rotate:PDFLib.degrees(angle)});
    });
    setProgress(80);await saveBytesToFile(new Blob([await doc.save()]),'Watermarked_'+t.name);
  }catch(e){showToast('Watermark failed.');}showLoading(false);
}
function modalHeaderFooter(){
  showModal('Header & Footer',
    '<label>Header ({page}, {total}, {date}):</label><input type="text" id="hf-header" value="Page {page} of {total}">'+
    '<label>Footer:</label><input type="text" id="hf-footer" value="{date}">'+
    '<label>Font size:</label><input type="number" id="hf-size" value="9" min="6" max="24">'+
    '<div class="modal-actions"><button class="btn btn-secondary" onclick="closeModal()">Cancel</button>'+
    '<button class="btn btn-primary" onclick="doHeaderFooter()">Apply</button></div>');
}
async function doHeaderFooter(){
  ...
  try{
    const doc=await PDFLib.PDFDocument.load(t.bytes);const pages=doc.getPages();const total=pages.length;
    ...
    setProgress(80);await saveBytesToFile(new Blob([await doc.save()]),'HF_'+t.name);
  }catch(e){showToast('Header/Footer failed.');}showLoading(false);
}
function modalBatesNumber(){
  showModal('Bates Numbering',
    '<label>Prefix:</label><input type="text" id="bates-prefix" value="DOC-">'+
    '<label>Start number:</label><input type="number" id="bates-start" value="1" min="0">'+
    '<label>Digits:</label><input type="number" id="bates-digits" value="6" min="1" max="10">'+
    '<label>Position:</label><select id="bates-pos"><option value="br">Bottom Right</option>'+
    '<option value="bl">Bottom Left</option><option value="tr">Top Right</option><option value="tl">Top Left</option></select>'+
    '<div class="modal-actions"><button class="btn btn-secondary" onclick="closeModal()">Cancel</button>'+
    '<button class="btn btn-primary" onclick="doBates()">Apply</button></div>');
}
async function doBates(){
  ...
  try{
    ...
    pages.forEach((page,i)=>{
      ...
      let x,y;
      if(pos==='br'){x=width-80;y=12;}else if(pos==='bl'){x=20;y=12;}
      else if(pos==='tr'){x=width-80;y=height-20;}else{x=20;y=height-20;}
      page.drawText(label,{x,y,size:8,color:PDFLib.rgb(.2,.2,.2)});
    });
    setProgress(80);await saveBytesToFile(new Blob([await doc.save()]),'Bates_'+t.name);
  }catch(e){showToast('Bates numbering failed.');}showLoading(false);
}
function modalPassword(){
  showModal('Encrypt PDF',
    '<p style="font-size:11px;color:var(--c-muted);margin-bottom:10px;">Note: AES-256 requires server-side processing.</p>'+
    '<label>User password:</label><input type="password" id="pw-user">'+
    '<label>Owner password:</label><input type="password" id="pw-owner">'+
    '<div class="modal-actions"><button class="btn btn-secondary" onclick="closeModal()">Cancel</button>'+
    '<button class="btn btn-primary" onclick="doPassword()">Apply</button></div>');
}
async function doPassword(){closeModal();showToast('PDF encryption requires server-side processing in this build. Saved as-is.','warn');}
async function actionCompressPDF(){
  const t=activeTab();if(!t){showToast('Open a PDF.');return;}
  showLoading(true,'Compressing...',20);
  try{
    const doc=await PDFLib.PDFDocument.load(t.bytes,{ignoreEncryption:true});setProgress(60);
    const saved=await doc.save({useObjectStreams:true,addDefaultPage:false});
    const ratio=Math.round((1-(saved.length/t.bytes.length))*100);setProgress(90);
    await saveBytesToFile(new Blob([saved]),'Compressed_'+t.name);
    showToast('Compressed! Reduction: '+(ratio>0?ratio+'%':'minimal.'),'success');
  }catch(e){showToast('Compress failed.');}showLoading(false);
}
</script>
)JS";
...
async function actionPDFtoImages(){
  ...
  try{
    ...
    setProgress(95);const zipBlob=await zip.generateAsync({type:'blob'});
    await saveBytesToFile(zipBlob,'Pages_'+t.name.replace('.pdf','')+'.zip','zip','application/zip');
  }catch(e){showToast('Export failed.');}showLoading(false);
}
async function actionPDFtoText(){
  ...
  }catch(e){showToast('Text export failed.');}showLoading(false);
}
async function actionPerformOCR(){
  ...
  try{
    ...
    const c=document.createElement('canvas');c.width=vp.width;c.height=vp.height;
    await page.render({canvasContext:c.getContext('2d'),viewport:vp}).promise;setProgress(40);
    const result=await Tesseract.recognize(c.toDataURL('image/png'),'eng');setProgress(90);
  ...
  }catch(e){showToast('OCR failed: '+e.message);}showLoading(false);
}
async function refreshStats(){
  const t=activeTab();if(!t)return;
  let words=0,chars=0;
  try{for(let i=0;i<Math.min(t.pageOrder.length,5);i++){
    const page=await t.doc.getPage(t.pageOrder[i]);
    const content=await page.getTextContent();const text=content.items.map(x=>x.str).join(' ');
  ...
  document.getElementById('stat-annots').textContent=(t.annotations.length+t.textBoxes.length+t.pasteImages.length+t.redactions.length).toLocaleString();
...
function cycleViewMode(){
  g_viewMode=(g_viewMode+1)%3;document.body.classList.remove('night','sepia');
...
function enterReadMode(){
  document.body.classList.toggle('read');const on=document.body.classList.contains('read');
...
function enterPresentation(){
  if(document.body.classList.contains('present')){
    document.body.classList.remove('present');document.getElementById('sb-mode').textContent='Normal';document.exitFullscreen?.();
  }else{
    document.body.classList.add('present');document.getElementById('sb-mode').textContent='Presentation';document.documentElement.requestFullscreen?.();
  }
}
function toggleRuler(){g_showRuler=!g_showRuler;showToast('Ruler: '+(g_showRuler?'On':'Off'));}
function toggleGrid(){g_showGrid=!g_showGrid;document.querySelectorAll('.grid-overlay').forEach(g=>g.classList.toggle('show',g_showGrid));showToast('Grid: '+(g_showGrid?'On':'Off'));}
async function showDocProperties(){
  const t=activeTab();if(!t){showToast('No file open.');return;}
  let title='-',author='-',subject='-',creator='-';
  try{const meta=await t.doc.getMetadata();if(meta&&meta.info){title=meta.info.Title||'-';author=meta.info.Author||'-';subject=meta.info.Subject||'-';creator=meta.info.Creator||'-';}}catch(e){}
  showModal('Document Properties',
    '<table style="width:100%;font-size:11px;border-collapse:collapse;">'+
    '<tr><td style="padding:4px 8px;color:var(--c-muted);width:90px;">File</td><td>'+t.name+'</td></tr>'+
    '<tr><td style="padding:4px 8px;color:var(--c-muted);">Pages</td><td>'+t.pageOrder.length+'</td></tr>'+
    '<tr><td style="padding:4px 8px;color:var(--c-muted);">Title</td><td>'+title+'</td></tr>'+
    '<tr><td style="padding:4px 8px;color:var(--c-muted);">Author</td><td>'+author+'</td></tr>'+
    '<tr><td style="padding:4px 8px;color:var(--c-muted);">Subject</td><td>'+subject+'</td></tr>'+
    '<tr><td style="padding:4px 8px;color:var(--c-muted);">Creator</td><td>'+creator+'</td></tr>'+
    '<tr><td style="padding:4px 8px;color:var(--c-muted);">Size</td><td>'+(t.bytes.length/1024).toFixed(1)+' KB</td></tr></table>'+
...
// ─────────────────────────────────────────────────────────────
// PART 36 · JS: Shortcuts modal, drag-drop, init
// ─────────────────────────────────────────────────────────────
ss << LR"JS(
<script>
function showShortcutModal(){
  showModal('Keyboard Shortcuts',
    '<table style="width:100%;font-size:11px;border-collapse:collapse;">'+
    '<tr><td style="padding:3px 8px;color:var(--c-muted);">1</td><td>Hand tool</td></tr>'+
    '<tr><td style="padding:3px 8px;color:var(--c-muted);">2</td><td>Select tool</td></tr>'+
    '<tr><td style="padding:3px 8px;color:var(--c-muted);">3</td><td>Pen tool</td></tr>'+
    '<tr><td style="padding:3px 8px;color:var(--c-muted);">4</td><td>Highlight tool</td></tr>'+
    '<tr><td style="padding:3px 8px;color:var(--c-muted);">5</td><td>Eraser</td></tr>'+
    '<tr><td style="padding:3px 8px;color:var(--c-muted);">6</td><td>Sticky Note</td></tr>'+
    '<tr><td style="padding:3px 8px;color:var(--c-muted);">7</td><td>Text Box</td></tr>'+
    '<tr><td style="padding:3px 8px;color:var(--c-muted);">8</td><td>Stamp</td></tr>'+
    '<tr><td style="padding:3px 8px;color:var(--c-muted);">R</td><td>Rotate all pages</td></tr>'+
    '<tr><td style="padding:3px 8px;color:var(--c-muted);">Del</td><td>Delete selected annotation</td></tr>'+
    '<tr><td style="padding:3px 8px;color:var(--c-muted);">Ctrl+O</td><td>Open file</td></tr>'+
    '<tr><td style="padding:3px 8px;color:var(--c-muted);">Ctrl+S</td><td>Save</td></tr>'+
    '<tr><td style="padding:3px 8px;color:var(--c-muted);">Ctrl+Z</td><td>Undo</td></tr>'+
    '<tr><td style="padding:3px 8px;color:var(--c-muted);">Ctrl+Y</td><td>Redo</td></tr>'+
    '<tr><td style="padding:3px 8px;color:var(--c-muted);">Ctrl+F</td><td>Find text</td></tr>'+
    '<tr><td style="padding:3px 8px;color:var(--c-muted);">Ctrl+Wheel</td><td>Smooth zoom</td></tr>'+
    '<tr><td style="padding:3px 8px;color:var(--c-muted);">Esc</td><td>Close dialogs</td></tr></table>'+
    '<div class="modal-actions"><button class="btn btn-secondary" onclick="closeModal()">Close</button></div>',true);
}
const va2=document.getElementById('viewer-area');
va2.addEventListener('dragover',e=>{e.preventDefault();va2.style.outline='3px dashed var(--c-accent2)';});
va2.addEventListener('dragleave',()=>{va2.style.outline='';});
va2.addEventListener('drop',async e=>{
  e.preventDefault();va2.style.outline='';
...
window.addEventListener('resize',()=>{
  if(g_resizeTimer)clearTimeout(g_resizeTimer);g_resizeTimer=setTimeout(()=>scheduleRender(),150);
});
window.loadPdfFromPath=loadPdfFromPath;
window.g_webViewController_bridge=true;
setTool('hand');renderTabStrip();updateStatusBar();
</script>
</body>
</html>
)JS";
