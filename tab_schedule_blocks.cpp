...
static void DrawSchOverlaySpinner(Graphics& g, float x, float y, const std::wstring& value, bool hMinus, bool hPlus, Font* fIcon, Font* fBold) {
    ...
    RectF minusRect(x, y, 36.0f, 36.0f); GraphicsPath* mp = GetSchRoundRectPath(minusRect, 6);
    SolidBrush mBr(hMinus ? colTealHover : colTeal); g.FillPath(&mBr, mp); delete mp; g.DrawString(L"-", -1, fBold, minusRect, &fC, &bWhite);

    RectF valRect(x + 40.0f, y, 50.0f, 36.0f); GraphicsPath* vp = GetSchRoundRectPath(valRect, 4);
    ...
    RectF plusRect(x + 94.0f, y, 36.0f, 36.0f); GraphicsPath* pp = GetSchRoundRectPath(plusRect, 6);
...
void ApplyProfileBlocking(int profileIdx, bool enable) {
    ...
    ApplyHostsFileBlocking(allPatterns, enable); ApplyPACFileBlocking(allPatterns, enable);
    ...
    while (true) {
        ...
        for (size_t i = 0; i < g_profiles.size(); ++i) {
            ...
            if (hasSchedule && p.lockMode == 0 && p.lockEndTime == 0) {
                ...
                if (p.activeDays[currentDay]) { shouldBeActive = (startTotalMins <= endTotalMins) ? (currentTotalMins >= startTotalMins && currentTotalMins < endTotalMins) : (currentTotalMins >= startTotalMins || currentTotalMins < endTotalMins); }
...
void DrawScheduleBlocksTab(Graphics& g, float x, float y, float w, float h) {
    ...
    FontFamily ffi(L"Segoe MDL2 Assets"); Font fIcon(&ffi, 20, FontStyleRegular, UnitPixel); Font fSmallIcon(&ffi, 14, FontStyleRegular, UnitPixel);
    ...
    RectF addBtnRect(x + w - 220, y + 20, 200, 40); GraphicsPath* aP = GetSchRoundRectPath(addBtnRect, 4);
    ...
    float cardW = (w - 60.0f) / 2.0f; float cardH = 170.0f; float startX = x + 20.0f; float startY = y + 100.0f - sch_cScroll;
    Region oldClip; g.GetClip(&oldClip); g.SetClip(RectF(x, y + 95.0f, w, h - 95.0f));

    for (size_t i = 0; i < g_profiles.size(); ++i) {
        float cX = startX + (i % 2) * (cardW + 20.0f); float cY = startY + (i / 2) * (cardH + 20.0f);
        if (cY > y + h || cY + cardH < y + 90.0f) continue; 
        
        RectF cardRect(cX, cY, cardW, cardH); GraphicsPath* cP = GetSchRoundRectPath(cardRect, 6);
        ...
        RectF togRect(cX + 15, cY + 115, 50, 26); GraphicsPath* tp = GetSchRoundRectPath(togRect, 13);
        SolidBrush tBg(g_profiles[i].isActive ? ClrGreen : ClrGrayText); g.FillPath(&tBg, tp); delete tp;
        g.FillEllipse(&bWhite, g_profiles[i].isActive ? (cX + 15.0f + 50.0f - 24.0f) : (cX + 15.0f + 2.0f), cY + 115.0f + 2.0f, 22.0f, 22.0f);
        ...
        RectF editRect(cX + cardW - 130, cY + 115, 60, 30); GraphicsPath* ep = GetSchRoundRectPath(editRect, 4);
        ...
        RectF delRect(cX + cardW - 60, cY + 115, 45, 30); GraphicsPath* dp = GetSchRoundRectPath(delRect, 4);
    ...
    if (editingProfileIdx != -1) {
        ...
        float tX = ovX + 25; float tY = ovY + 20; 
        for (int i = 0; i < 4; i++) {
            g_ehb.subTabRects[i] = RectF(tX, tY, 130, 28); GraphicsPath* tabP = GetSchRoundRectPath(g_ehb.subTabRects[i], 14);
        ...
        float contentY = ovY + 70.0f; 
        g.DrawLine(&pThin, ovX, contentY - 15.0f, ovX + ovW, contentY - 15.0f);
        g.DrawLine(&pThin, ovX, ovY + ovH - 65, ovX + ovW, ovY + ovH - 65);
        
        if (s_activeSubTab > 0) {
            g_ehb.backBtn = RectF(ovX + 25, ovY + ovH - 50, 90, 35); GraphicsPath* bbp = GetSchRoundRectPath(g_ehb.backBtn, 4);
        ...
        if (s_activeSubTab < 3) {
            g_ehb.nextBtn = RectF(ovX + 125, ovY + ovH - 50, 90, 35); GraphicsPath* nbp = GetSchRoundRectPath(g_ehb.nextBtn, 4);
        ...
        g_ehb.saveBtn = RectF(ovX + ovW - 140, ovY + ovH - 50, 110, 35); GraphicsPath* svp = GetSchRoundRectPath(g_ehb.saveBtn, 4);
        ...
        g_ehb.cancelBtn = RectF(ovX + ovW - 250, ovY + ovH - 50, 100, 35); GraphicsPath* cvp = GetSchRoundRectPath(g_ehb.cancelBtn, 4);
        ...
        float cardX = ovX + 25; float cardW_inner = ovW - 50; 

        // ================== TAB 0: BASIC & TIME ==================
        if (s_activeSubTab == 0) {
            RectF c1Rect(cardX, contentY, cardW_inner, 75); GraphicsPath* c1P = GetSchRoundRectPath(c1Rect, 6); g.FillPath(&bWhite, c1P); delete c1P;
            ...
            g_ehb.nameInp = RectF(cardX + 115, contentY + 32, 200, 32); GraphicsPath* np = GetSchRoundRectPath(g_ehb.nameInp, 4);
            ...
            if(inpProfileName.empty() && activeInput != 1) g.DrawString(L"e.g. Study Time", -1, &fNorm, g_ehb.nameInp, &fC, &bGray);
                ...
                if(activeInput == 1 && (GetTickCount()/500)%2==0) { Graphics gT(GetDesktopWindow()); RectF bR; gT.MeasureString(inpProfileName.c_str(), -1, &fNorm, PointF(0,0), &bR); g.FillRectangle(&bDark, g_ehb.nameInp.X+10+(inpProfileName.empty()?0:bR.Width), g_ehb.nameInp.Y+6, 1.5f, 20.0f); }
            }

            g.DrawString(L"Lock Mode:", -1, &fNorm, RectF(cardX + 340, contentY + 35, 80, 30), &fL, &bGray);
            g_ehb.modeDrop = RectF(cardX + 420, contentY + 32, 180, 32); GraphicsPath* mdp = GetSchRoundRectPath(g_ehb.modeDrop, 4);
            ...
            contentY += 90;
            RectF c2Rect(cardX, contentY, cardW_inner, 100); GraphicsPath* c2P = GetSchRoundRectPath(c2Rect, 6); g.FillPath(&bWhite, c2P); delete c2P;
            ...
            for(int d=0; d<7; d++) {
                g_ehb.days[d] = RectF(cardX + 15 + (d * 34), contentY + 55, 30, 30); GraphicsPath* dP = GetSchRoundRectPath(g_ehb.days[d], 15);
            ...
            auto DrawModernTimeBox = [&](float tx, float ty, const wstring& lbl, int h, int m, RectF& hBox, RectF& mBox, RectF& ampmBtn, bool hH, bool hM, bool hAmPm) {
                ...
                hBox = RectF(tx + 45, ty, 35, 30); GraphicsPath hp; AddRoundedRectPath(hp, hBox.X, hBox.Y, hBox.Width, hBox.Height, 6); SolidBrush hbBr(hH ? ClrBgHover : ClrBg); g.FillPath(&hbBr, &hp); g.DrawPath(&pThin, &hp); g.DrawString((dispH < 10 ? L"0" + to_wstring(dispH) : to_wstring(dispH)).c_str(), -1, &fBold, hBox, &fC, &bDark);
                g.DrawString(L":", -1, &fBold, RectF(tx + 80, ty, 10, 30), &fC, &bDark);
                mBox = RectF(tx + 90, ty, 35, 30); GraphicsPath mp; AddRoundedRectPath(mp, mBox.X, mBox.Y, mBox.Width, mBox.Height, 6); SolidBrush mbBr(hM ? ClrBgHover : ClrBg); g.FillPath(&mbBr, &mp); g.DrawPath(&pThin, &mp); g.DrawString((m < 10 ? L"0" + to_wstring(m) : to_wstring(m)).c_str(), -1, &fBold, mBox, &fC, &bDark);
                ampmBtn = RectF(tx + 130, ty, 38, 30); GraphicsPath ap; AddRoundedRectPath(ap, ampmBtn.X, ampmBtn.Y, ampmBtn.Width, ampmBtn.Height, 6); SolidBrush aBr(hAmPm ? ClrTealHover : ClrBgHover); g.FillPath(&aBr, &ap); g.DrawPath(hAmPm ? &pTeal : &pThin, &ap); g.DrawString(ampmStr.c_str(), -1, &fBold, ampmBtn, &fC, (h >= 12) ? &bTeal : &bDark);
            };
        ...
        // ================== TAB 1: QUICK SETTINGS ==================
            ...
            RectF c3Rect(cardX, contentY, cardW_inner, 75); GraphicsPath* c3P = GetSchRoundRectPath(c3Rect, 6); g.FillPath(&bWhite, c3P); g.DrawPath(&pThin, c3P); delete c3P;
            float cbWidth = (cardW_inner - 20.0f) / 2.0f; 

            auto DrawCb = [&](RectF& outHitbox, float cx, const wstring& label, bool val, bool hov) {
                outHitbox = RectF(cx, contentY + 17, cbWidth - 10, 40); 
                if (hov) { GraphicsPath* hp = GetSchRoundRectPath(outHitbox, 4); g.FillPath(&bBgHover, hp); delete hp; }
                RectF cbBox(cx + 10, contentY + 27, 20, 20); GraphicsPath* bp = GetSchRoundRectPath(cbBox, 4); g.FillPath(val ? &bTeal : &bWhite, bp); g.DrawPath(val ? &pTeal : &pThin, bp); delete bp;
            ...
            DrawCb(g_ehb.togUni, cardX + 10 + cbWidth, L"Block Uninstall / Taskmgr", editBlockUninst, g_ehb.hTogUni);
            
            contentY += 90;
            RectF c4Rect(cardX, contentY, cardW_inner, 85); GraphicsPath* c4P = GetSchRoundRectPath(c4Rect, 6); g.FillPath(&bWhite, c4P); g.DrawPath(&pThin, c4P); delete c4P;
            ...
            float qbX = cardX + 15; float qbY = contentY + 40; float qbW = 110.0f; float qbH = 30.0f; float qbGap = 10.0f;
        ...
        // ================== TAB 2: CUSTOM LISTS (2 Columns Wide) ==================
            ...
            RectF cRect(cardX, contentY, cardW_inner, listAreaH); GraphicsPath* cP = GetSchRoundRectPath(cRect, 6); g.FillPath(&bWhite, cP); delete cP;

            float colGap = 20.0f; float colW = (cardW_inner - (colGap * 2)) / 2.0f;
            ...
            auto DrawListCol = [&](int colIdx, float colX, const wstring& title, const wstring& ph, wstring& inpStr, int inpIdx, vector<SchBlockItem>* list,
                ...
                float inpY = contentY + 45.0f; float comboW = outCombo ? 30.0f : 0.0f; float addW = 65.0f; float gap = 5.0f;
                float inpW = colW - comboW - addW - (outCombo ? gap * 2 : gap);

                outInp = RectF(colX, inpY, inpW, 36); GraphicsPath* ip = GetSchRoundRectPath(outInp, 4);
                g.FillPath(activeInput == inpIdx ? &bWhite : &bBg, ip); g.DrawPath(activeInput == inpIdx ? &pTeal : &pThin, ip); delete ip;
                if(inpStr.empty() && activeInput != inpIdx) { g.DrawString(ph.c_str(), -1, &fNorm, RectF(outInp.X+8, outInp.Y, outInp.Width-16, outInp.Height), &fL, &bGray); } 
                else { g.DrawString(inpStr.c_str(), -1, &fNorm, RectF(outInp.X+8, outInp.Y, outInp.Width-16, outInp.Height), &fL, &bDark);
                    if(activeInput == inpIdx && (GetTickCount()/500)%2==0) { Graphics gT(GetDesktopWindow()); RectF bR; gT.MeasureString(inpStr.c_str(), -1, &fNorm, PointF(0,0), &bR); g.FillRectangle(&bDark, outInp.X+8+(inpStr.empty()?0:bR.Width), outInp.Y+8, 1.5f, 20.0f); }
                }

                float nextX = outInp.X + outInp.Width + gap;
                if(outCombo) {
                    *outCombo = RectF(nextX, inpY, comboW, 36); GraphicsPath* cbp = GetSchRoundRectPath(*outCombo, 4); SolidBrush cbBr(hovCombo ? ClrBgHover : ClrWhite); g.FillPath(&cbBr, cbp); g.DrawPath(&pThin, cbp); delete cbp; g.DrawString(L"\xE70D", -1, &fSmallIcon, *outCombo, &fC, &bDark);
                    nextX += comboW + gap;
                }
                outAdd = RectF(nextX, inpY, addW, 36); GraphicsPath* ap = GetSchRoundRectPath(outAdd, 4); SolidBrush aBr(hovAdd ? ClrTealHover : ClrTeal); g.FillPath(&aBr, ap); delete ap; g.DrawString(L"+ Add", -1, &fBold, outAdd, &fC, &bWhite);

                outDel.clear(); float listStartY = inpY + 45.0f; float boxH = listAreaH - 100.0f;
                ...
                if(list && !list->empty()) {
                    s_listScrollMax[colIdx] = (std::max)(0.0f, (list->size() * 35.0f) - boxH + 10.0f); s_listScrollC[colIdx] += (s_listScrollT[colIdx] - s_listScrollC[colIdx]) * 0.15f;
                    ...
                    for(size_t i = 0; i < list->size(); ++i) {
                        ...
                        if (itemY + 30 > listStartY && itemY < listStartY + boxH) {
                            RectF rowR(colX + 5, itemY, colW - 10, 30); g.DrawString(item.name.c_str(), -1, &fNorm, RectF(rowR.X+5, rowR.Y, rowR.Width-35, rowR.Height), &fL, &bDark);
                            RectF delR(rowR.X + rowR.Width - 30, rowR.Y, 30, 30); outDel.push_back({delR, (int)i}); SolidBrush crBr(item.isHoveredCross ? ClrRed : ClrGrayText); g.DrawString(L"\xE711", -1, &fSmallIcon, delR, &fC, &crBr);
                    ...
                    if (s_listScrollMax[colIdx] > 0) {
                        ...
                        RectF thumbR(colX + colW - 6, thumbY, 4, thumbH); GraphicsPath thP; AddRoundedRectPath(thP, thumbR.X, thumbR.Y, thumbR.Width, thumbR.Height, 2); SolidBrush thB(Color(100, 150, 150, 150)); g.FillPath(&thB, &thP);
                    }
            ...
            float btnW = (colW - 10.0f) / 3.0f; float btnY = contentY + listAreaH - 45.0f; float appColX = startColX + colW + colGap;

            g_ehb.btnAddExe = RectF(appColX, btnY, btnW, 36.0f); GraphicsPath* p1 = GetSchRoundRectPath(g_ehb.btnAddExe, 4); SolidBrush b1(g_ehb.hBtnAddExe ? ClrGreen : Color(255, 90, 170, 20)); g.FillPath(&b1, p1); delete p1; g.DrawString(L"Add Exe", -1, &fSmallBold, g_ehb.btnAddExe, &fC, &bWhite);
            g_ehb.btnAddStore = RectF(appColX + btnW + 5.0f, btnY, btnW, 36.0f); GraphicsPath* p2 = GetSchRoundRectPath(g_ehb.btnAddStore, 4); SolidBrush b2(g_ehb.hBtnAddStore ? ClrGreen : Color(255, 90, 170, 20)); g.FillPath(&b2, p2); delete p2; g.DrawString(L"Add Store", -1, &fSmallBold, g_ehb.btnAddStore, &fC, &bWhite);
            g_ehb.btnAddTitle = RectF(appColX + (btnW * 2) + 10.0f, btnY, btnW, 36.0f); GraphicsPath* p3 = GetSchRoundRectPath(g_ehb.btnAddTitle, 4); SolidBrush b3(g_ehb.hBtnAddTitle ? ClrGreen : Color(255, 90, 170, 20)); g.FillPath(&b3, p3); delete p3; g.DrawString(L"Add Title", -1, &fSmallBold, g_ehb.btnAddTitle, &fC, &bWhite);
        }
        ...
        // ================== TAB 3: ADULT BLOCK (NEW) ==================
            ...
            float colGap = 20.0f; float colW = (cardW_inner - (colGap * 2)) / 2.0f; float startColX = cardX + 15.0f;

            g.DrawString(L"Safe Browsing Rules", -1, &fCardTitle, RectF(startColX, contentY + 10, colW, 30), &fL, &bDark);
            ...
            auto DrawRuleCb = [&](RectF& outHitbox, float cy, const wstring& label, bool val, bool hov) {
                outHitbox = RectF(startColX, cy, colW - 10, 40); 
                if (hov) { GraphicsPath* hp = GetSchRoundRectPath(outHitbox, 4); g.FillPath(&bBgHover, hp); delete hp; }
                RectF cbBox(startColX + 5, cy + 10, 20, 20); GraphicsPath* bp = GetSchRoundRectPath(cbBox, 4); g.FillPath(val ? &bTeal : &bWhite, bp); g.DrawPath(val ? &pTeal : &pThin, bp); delete bp;
                if(val) g.DrawString(L"\xE73E", -1, &fSmallIcon, cbBox, &fC, &bWhite);
                g.DrawString(label.c_str(), -1, &fNorm, RectF(startColX + 35, cy + 10, colW - 45, 20), &fL, &bDark);
            };

            DrawRuleCb(g_ehb.cbAdultWeb, contentY + 50, L"Block Adult Websites (Resource List)", cA, g_ehb.hCbAdultWeb);
            DrawRuleCb(g_ehb.cbHardcore, contentY + 95, L"Block Hardcore Keywords (Porn, NSFW)", cH, g_ehb.hCbHardcore);
            DrawRuleCb(g_ehb.cbRomantic, contentY + 140, L"Block Romantic / Softcore Content", cR, g_ehb.hCbRomantic);
            DrawRuleCb(g_ehb.cbStrictDns, contentY + 185, L"Enforce Strict DNS & SafeSearch", cD, g_ehb.hCbStrictDns);

            float rightColX = startColX + colW + colGap;
            g.DrawString(L"Custom Bad Words", -1, &fCardTitle, RectF(rightColX, contentY + 10, colW, 30), &fL, &bDark);

            float inpY = contentY + 45.0f; float addW = 65.0f; float gap = 5.0f; float inpW = colW - addW - gap;
            g_ehb.keyInp = RectF(rightColX, inpY, inpW, 36); GraphicsPath* ip = GetSchRoundRectPath(g_ehb.keyInp, 4);
            g.FillPath(activeInput == 4 ? &bWhite : &bBg, ip); g.DrawPath(activeInput == 4 ? &pTeal : &pThin, ip); delete ip;
            if(inpKey.empty() && activeInput != 4) { g.DrawString(L"e.g. badword", -1, &fNorm, RectF(g_ehb.keyInp.X+8, g_ehb.keyInp.Y, g_ehb.keyInp.Width-16, g_ehb.keyInp.Height), &fL, &bGray); } 
            else { g.DrawString(inpKey.c_str(), -1, &fNorm, RectF(g_ehb.keyInp.X+8, g_ehb.keyInp.Y, g_ehb.keyInp.Width-16, g_ehb.keyInp.Height), &fL, &bDark);
                if(activeInput == 4 && (GetTickCount()/500)%2==0) { Graphics gT(GetDesktopWindow()); RectF bR; gT.MeasureString(inpKey.c_str(), -1, &fNorm, PointF(0,0), &bR); g.FillRectangle(&bDark, g_ehb.keyInp.X+8+(inpKey.empty()?0:bR.Width), g_ehb.keyInp.Y+8, 1.5f, 20.0f); }
            }

            g_ehb.addKey = RectF(g_ehb.keyInp.X + g_ehb.keyInp.Width + gap, inpY, addW, 36); GraphicsPath* ap = GetSchRoundRectPath(g_ehb.addKey, 4); SolidBrush aBr(g_ehb.hAddKey ? ClrTealHover : ClrTeal); g.FillPath(&aBr, ap); delete ap; g.DrawString(L"+ Add", -1, &fBold, g_ehb.addKey, &fC, &bWhite);

            g_ehb.keyDel.clear(); float listStartY = inpY + 45.0f; float boxH = listAreaH - 100.0f;
            ...
            if(aList && !aList->empty()) {
                s_listScrollMax[2] = (std::max)(0.0f, (aList->size() * 35.0f) - boxH + 10.0f); s_listScrollC[2] += (s_listScrollT[2] - s_listScrollC[2]) * 0.15f;
                g.SetClip(g_ehb.listAreas[2]); float itemY = listStartY + 5.0f - s_listScrollC[2];
                for(size_t i = 0; i < aList->size(); ++i) {
                    auto& item = (*aList)[i];
                    if (itemY + 30 > listStartY && itemY < listStartY + boxH) {
                        RectF rowR(rightColX + 5, itemY, colW - 10, 30); g.DrawString(item.name.c_str(), -1, &fNorm, RectF(rowR.X+5, rowR.Y, rowR.Width-35, rowR.Height), &fL, &bDark);
                        RectF delR(rowR.X + rowR.Width - 30, rowR.Y, 30, 30); g_ehb.keyDel.push_back({delR, (int)i}); SolidBrush crBr(item.isHoveredCross ? ClrRed : ClrGrayText); g.DrawString(L"\xE711", -1, &fSmallIcon, delR, &fC, &crBr);
                        g.DrawLine(&pThin, rowR.X, rowR.Y + 30.0f, rowR.X + rowR.Width, rowR.Y + 30.0f);
                    } itemY += 35.0f;
                } g.SetClip(&oldClip);
                if (s_listScrollMax[2] > 0) {
                    ...
                    RectF thumbR(rightColX + colW - 6, thumbY, 4, thumbH); GraphicsPath thP; AddRoundedRectPath(thP, thumbR.X, thumbR.Y, thumbR.Width, thumbR.Height, 2); SolidBrush thB(Color(100, 150, 150, 150)); g.FillPath(&thB, &thP);
                }
        ...
        if (s_activeSubTab == 0 && isSchModeDropdownOpen) {
            RectF mlR(g_ehb.modeDrop.X, g_ehb.modeDrop.Y + 34, 180, 118); GraphicsPath* mlP = GetSchRoundRectPath(mlR, 4); g.FillPath(&bWhite, mlP); g.DrawPath(&pThin, mlP); delete mlP;
            g_ehb.modeOpt[0] = RectF(mlR.X+2, mlR.Y+2, 176, 38); g_ehb.modeOpt[1] = RectF(mlR.X+2, mlR.Y+40, 176, 38); g_ehb.modeOpt[2] = RectF(mlR.X+2, mlR.Y+78, 176, 38);
            SolidBrush o1Br(g_ehb.hOptSelf ? ClrBgHover : ClrWhite); g.FillRectangle(&o1Br, g_ehb.modeOpt[0]); g.DrawString(L"Self Control", -1, &fNorm, RectF(mlR.X+10, mlR.Y+2, 160, 38), &fL, &bDark);
            SolidBrush o2Br(g_ehb.hOptParents ? ClrBgHover : ClrWhite); g.FillRectangle(&o2Br, g_ehb.modeOpt[1]); g.DrawString(L"Parents Control", -1, &fNorm, RectF(mlR.X+10, mlR.Y+40, 160, 38), &fL, &bDark);
            SolidBrush o3Br(g_ehb.hOptLongText ? ClrBgHover : ClrWhite); g.FillRectangle(&o3Br, g_ehb.modeOpt[2]); g.DrawString(L"Long Text Unlock", -1, &fNorm, RectF(mlR.X+10, mlR.Y+78, 160, 38), &fL, &bDark);
        }
        
        auto DrawDynamicDropdown = [&](RectF btnRect, vector<wstring>& opts, vector<RectF>& outOpts, int hovIdx) {
            RectF lR(btnRect.X - 120, btnRect.Y + 32, 145, opts.size() * 30 + 10); GraphicsPath* lP = GetSchRoundRectPath(lR, 4); g.FillPath(&bWhite, lP); g.DrawPath(&pThin, lP); delete lP;
            outOpts.clear(); float iY = lR.Y + 5;
            for(size_t i=0; i<opts.size(); ++i) {
                RectF optRect(lR.X+2, iY, lR.Width-4, 30); outOpts.push_back(optRect);
                SolidBrush oBr(hovIdx == (int)i ? ClrBgHover : ClrWhite); g.FillRectangle(&oBr, optRect); g.DrawString(opts[i].c_str(), -1, &fSmall, RectF(lR.X+5, iY, lR.Width-10, 30), &fL, &bDark); iY += 30;
            }
        };
        ...
        if (s_activeSubTab == 2) {
            ...
            if (isSchStoreComboOpen) {
                RectF lR(g_ehb.btnAddStore.X, g_ehb.btnAddStore.Y - (schCommonStoreApps.size() * 30 + 10), 145, schCommonStoreApps.size() * 30 + 10);
                GraphicsPath* lP = GetSchRoundRectPath(lR, 4); g.FillPath(&bWhite, lP); g.DrawPath(&pThin, lP); delete lP;
                g_ehb.storeOpts.clear(); float iY = lR.Y + 5;
                for(size_t i=0; i<schCommonStoreApps.size(); ++i) {
                    RectF optRect(lR.X+2, iY, lR.Width-4, 30); g_ehb.storeOpts.push_back(optRect);
                    SolidBrush oBr(hoverSchStoreOptIdx == (int)i ? ClrBgHover : ClrWhite); g.FillRectangle(&oBr, optRect); g.DrawString(schCommonStoreApps[i].c_str(), -1, &fSmall, RectF(lR.X+5, iY, lR.Width-10, 30), &fL, &bDark); iY += 30;
                }
            }
        }
    }
    ...
    if (s_showTimeOverlay || s_showPassOverlay || s_showTextUnlockOverlay) {
        ...
        float ovW = s_showTextUnlockOverlay ? 600.0f : 500.0f; float ovH = s_showTextUnlockOverlay ? 450.0f : 280.0f;
        float ovX = x + (w - ovW) / 2.0f; float ovY = y + (h - ovH) / 2.0f;
        RectF ovRect(ovX, ovY, ovW, ovH); GraphicsPath* op = GetSchRoundRectPath(ovRect, 8); g.FillPath(&bBg, op); g.DrawPath(&pThin, op); delete op;

        if (s_showTimeOverlay) {
            g.DrawString(L"SET FOCUS DURATION (SELF CONTROL)", -1, &fTitle, RectF(ovX, ovY + 20, ovW, 30), &fC, &bDark);
            g.DrawString(L"Months:", -1, &fBold, RectF(ovX + 40, ovY + 80, 60, 36), &fL, &bDark); DrawSchOverlaySpinner(g, ovX + 110, ovY + 80, to_wstring(s_focusMonths), s_hTimeMoM, s_hTimeMoP, &fIcon, &fBold);
            g.DrawString(L"Days:", -1, &fBold, RectF(ovX + 250, ovY + 80, 50, 36), &fL, &bDark); DrawSchOverlaySpinner(g, ovX + 300, ovY + 80, to_wstring(s_focusDays), s_hTimeDM, s_hTimeDP, &fIcon, &fBold);
            g.DrawString(L"Hours:", -1, &fBold, RectF(ovX + 40, ovY + 140, 60, 36), &fL, &bDark); DrawSchOverlaySpinner(g, ovX + 110, ovY + 140, to_wstring(s_focusHours), s_hTimeHM, s_hTimeHP, &fIcon, &fBold);
            g.DrawString(L"Mins:", -1, &fBold, RectF(ovX + 250, ovY + 140, 50, 36), &fL, &bDark); DrawSchOverlaySpinner(g, ovX + 300, ovY + 140, to_wstring(s_focusMins), s_hTimeMM, s_hTimeMP, &fIcon, &fBold);
            RectF cancelRect(ovX + 60, ovY + 210, 140, 40); GraphicsPath* cp = GetSchRoundRectPath(cancelRect, 4); SolidBrush cancelBrush(s_hTimeCancel ? ClrBgHover : ClrWhite); g.FillPath(&cancelBrush, cp); g.DrawPath(&pThin, cp); delete cp; g.DrawString(L"Cancel (Esc)", -1, &fBold, cancelRect, &fC, &bDark);
            RectF startRect(ovX + 240, ovY + 210, 140, 40); GraphicsPath* sp = GetSchRoundRectPath(startRect, 4); SolidBrush startBrush(s_hTimeStart ? ClrTealHover : ClrTeal); g.FillPath(&startBrush, sp); delete sp; g.DrawString(L"Start Profile", -1, &fBold, startRect, &fC, &bWhite);
        } else if (s_showPassOverlay) {
            wstring titleTxt = s_isStoppingFocus ? L"ENTER PARENTS PASSWORD TO STOP" : L"SET PARENTS PASSWORD"; g.DrawString(titleTxt.c_str(), -1, &fTitle, RectF(ovX, ovY + 20, ovW, 30), &fC, &bDark);
            RectF passInpRect(ovX + 40, ovY + 80, ovW - 80, 40); GraphicsPath* pp = GetSchRoundRectPath(passInpRect, 4); g.FillPath(s_isPassInputActive ? &bWhite : &bBg, pp); g.DrawPath(s_isPassInputActive ? &pTeal : &pThin, pp); delete pp;
            ...
            else { g.DrawString(displayPass.c_str(), -1, &fTitle, RectF(ovX + 50, ovY + 85, ovW - 100, 30), &fL, &bDark); if (s_isPassInputActive && (GetTickCount() / 500) % 2 == 0) { Graphics gT(GetDesktopWindow()); RectF bR; gT.MeasureString(displayPass.c_str(), -1, &fTitle, PointF(0,0), &bR); float curX = ovX + 52 + (displayPass.empty()?0:bR.Width); g.FillRectangle(&bDark, curX, ovY + 90, 1.5f, 20.0f); } }
            RectF cancelRect(ovX + 40, ovY + 150, 140, 40); GraphicsPath* cp = GetSchRoundRectPath(cancelRect, 4); SolidBrush cancelBrush(s_hPassCancel ? ClrBgHover : ClrWhite); g.FillPath(&cancelBrush, cp); g.DrawPath(&pThin, cp); delete cp; g.DrawString(L"Cancel (Esc)", -1, &fBold, cancelRect, &fC, &bDark);
            RectF confRect(ovX + 220, ovY + 150, 160, 40); GraphicsPath* sp = GetSchRoundRectPath(confRect, 4); SolidBrush confBrush(s_hPassConfirm ? ClrTealHover : ClrTeal); g.FillPath(&confBrush, sp); delete sp; g.DrawString(L"Confirm", -1, &fBold, confRect, &fC, &bWhite);
        } else if (s_showTextUnlockOverlay) {
            g.DrawString(L"EXACT TEXT UNLOCK MODE", -1, &fTitle, RectF(ovX, ovY + 20, ovW, 30), &fC, &bDark);
            RectF targetBox(ovX + 20, ovY + 60, ovW - 40, 110); GraphicsPath* tbp = GetSchRoundRectPath(targetBox, 4); g.FillPath(&bWhite, tbp); g.DrawPath(&pThin, tbp); delete tbp; g.DrawString(s_targetUnlockText.c_str(), -1, &fNorm, RectF(targetBox.X+8, targetBox.Y+8, targetBox.Width-16, targetBox.Height-16), &fTL, &bGray);
            RectF typeBox(ovX + 20, ovY + 180, ovW - 40, 160); GraphicsPath* tp = GetSchRoundRectPath(typeBox, 4); g.FillPath(s_isTypingActive ? &bWhite : &bBg, tp); g.DrawPath(s_isTypingActive ? &pTeal : &pThin, tp); delete tp;
            ...
            RectF cancelRect(ovX + 120, ovY + 380, 140, 40); GraphicsPath* cp = GetSchRoundRectPath(cancelRect, 4); SolidBrush cancelBrush(s_hTextUnlockCancel ? ClrBgHover : ClrWhite); g.FillPath(&cancelBrush, cp); g.DrawPath(&pThin, cp); delete cp; g.DrawString(L"Cancel (Esc)", -1, &fBold, cancelRect, &fC, &bDark);
            RectF confRect(ovX + 280, ovY + 380, 160, 40); GraphicsPath* sp = GetSchRoundRectPath(confRect, 4); SolidBrush confBrush((s_currentTypingText == s_targetUnlockText) ? (s_hTextUnlockConfirm ? ClrTealHover : ClrTeal) : ClrDisabled); g.FillPath(&confBrush, sp); delete sp; g.DrawString(L"Unlock Profile", -1, &fBold, confRect, &fC, &bWhite);
        }
    }
}
...
void ProcessScheduleBlocksMouseMove(float x, float y) {
    ...
    if (s_showTimeOverlay || s_showPassOverlay || s_showTextUnlockOverlay) {
        float ovW = s_showTextUnlockOverlay ? 600.0f : 500.0f; float ovH = s_showTextUnlockOverlay ? 450.0f : 280.0f; float ovX = s_cx + (s_cw - ovW) / 2.0f; float ovY = s_cy + (s_ch - ovH) / 2.0f;
        if (s_showTimeOverlay) {
            if (RectF(ovX+110, ovY+80, 32, 36).Contains(x,y)) s_hTimeMoM=true; if (RectF(ovX+202, ovY+80, 32, 36).Contains(x,y)) s_hTimeMoP=true;
            if (RectF(ovX+300, ovY+80, 32, 36).Contains(x,y)) s_hTimeDM=true; if (RectF(ovX+392, ovY+80, 32, 36).Contains(x,y)) s_hTimeDP=true;
            if (RectF(ovX+110, ovY+140, 32, 36).Contains(x,y)) s_hTimeHM=true; if (RectF(ovX+202, ovY+140, 32, 36).Contains(x,y)) s_hTimeHP=true;
            if (RectF(ovX+300, ovY+140, 32, 36).Contains(x,y)) s_hTimeMM=true; if (RectF(ovX+392, ovY+140, 32, 36).Contains(x,y)) s_hTimeMP=true;
            if (RectF(ovX+60, ovY+210, 140, 40).Contains(x,y)) s_hTimeCancel=true; if (RectF(ovX+240, ovY+210, 140, 40).Contains(x,y)) s_hTimeStart=true;
        } else if (s_showPassOverlay) {
            if (RectF(ovX+40, ovY+80, ovW-80, 40).Contains(x,y)) s_hPassInput=true; if (RectF(ovX+40, ovY+150, 140, 40).Contains(x,y)) s_hPassCancel=true; if (RectF(ovX+220, ovY+150, 160, 40).Contains(x,y)) s_hPassConfirm=true;
        } else if (s_showTextUnlockOverlay) {
            if (RectF(ovX+120, ovY+380, 140, 40).Contains(x,y)) s_hTextUnlockCancel=true; if (RectF(ovX+280, ovY+380, 160, 40).Contains(x,y)) s_hTextUnlockConfirm=true;
        }
        return;
    }
    ...
    if (editingProfileIdx != -1) {
        ...
        if (editingProfileIdx >= 0) { for(auto& it : g_profiles[editingProfileIdx].blockedWebsites) it.isHoveredCross=false; for(auto& it : g_profiles[editingProfileIdx].blockedApps) it.isHoveredCross=false; for(auto& it : g_profiles[editingProfileIdx].adultCustomKeywords) it.isHoveredCross=false; }
        ...
        if (s_activeSubTab == 0) {
            ...
            for (size_t qi=0; qi<s_quickBlocks.size() && qi<s_quickBlockRects.size(); ++qi) { if (s_quickBlockRects[qi].Contains(x, y)) s_quickBlocks[qi].hovered = true; }
        } 
        else if (s_activeSubTab == 2) {
    ...
    if (s_scrollbarDragging) {
        ...
        float thumbRange = (s_ch - 100.0f) - thumbH; float dy = y - s_scrollbarDragStartY;
    ...
    float cardW = (s_cw - 60.0f) / 2.0f; float cardH = 170.0f; float startX = s_cx + 20.0f; float startY = s_cy + 100.0f - sch_cScroll;
    for (size_t i = 0; i < g_profiles.size(); ++i) {
        float cX = startX + (i % 2) * (cardW + 20.0f); float cY = startY + (i / 2) * (cardH + 20.0f); if (cY > s_cy + s_ch || cY + cardH < s_cy + 90.0f) continue;
        ...
        if(!g_profiles[i].isActive) g_profiles[i].hDel = RectF(cX + cardW - 60, cY + 115, 45, 30).Contains(x, y); else g_profiles[i].hDel = false; 
    }
}
...
void ProcessScheduleBlocksMouseClick(float x, float y) {
    ...
    if (editingProfileIdx != -1) {
        ...
        if (s_activeSubTab == 0) {
            ...
            if (editingProfileIdx >= 0) {
                if (g_ehb.hAddWeb && !inpWeb.empty()) { g_profiles[editingProfileIdx].blockedWebsites.push_back({inpWeb, false}); inpWeb = L""; }
                if (g_ehb.hAddApp && !inpApp.empty()) { if (inpApp.length() < 4 || inpApp.substr(inpApp.length() - 4) != L".exe") inpApp += L".exe"; g_profiles[editingProfileIdx].blockedApps.push_back({inpApp, false}); inpApp = L""; }
                if (g_ehb.listAreas[0].Contains(x, y)) { auto& webs = g_profiles[editingProfileIdx].blockedWebsites; for(auto& p : g_ehb.webDel) { if(p.first.Contains(x,y)) webs.erase(webs.begin() + p.second); } }
                if (g_ehb.listAreas[1].Contains(x, y)) { auto& apps = g_profiles[editingProfileIdx].blockedApps; for(auto& p : g_ehb.appDel) { if(p.first.Contains(x,y)) apps.erase(apps.begin() + p.second); } }
            }
        }
        else if (s_activeSubTab == 3) {
            if (editingProfileIdx >= 0) {
                ...
                activeInput = 0; if (g_ehb.keyInp.Contains(x,y)) activeInput = 4;
                if (g_ehb.hAddKey && !inpKey.empty()) { g_profiles[editingProfileIdx].adultCustomKeywords.push_back({inpKey, false}); inpKey = L""; }
                if (g_ehb.listAreas[2].Contains(x, y)) { auto& keys = g_profiles[editingProfileIdx].adultCustomKeywords; for(auto& p : g_ehb.keyDel) { if(p.first.Contains(x,y)) keys.erase(keys.begin() + p.second); } }
            }
        }
        return;
    }
    ...
    if (hAddProfileBtn) {
        ...
        s_listScrollT[0] = s_listScrollT[1] = s_listScrollT[2] = s_listScrollT[3] = 0; s_listScrollC[0] = s_listScrollC[1] = s_listScrollC[2] = s_listScrollC[3] = 0;
...
void ProcessScheduleBlocksMouseWheel(float x, float y, int delta) {
    ...
    float totalRows = ceil((float)g_profiles.size() / 2.0f); float maxScroll = (std::max)(0.0f, (totalRows * 190.0f) - (s_ch - 100.0f));
